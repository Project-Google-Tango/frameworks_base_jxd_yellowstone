...
class EGLLogWrapper implements EGL11 {
    ...
    public boolean eglPostSubBufferNV(EGLDisplay display, EGLSurface surface, int x, int y, int width, int height) {
        begin("eglPostSubBufferNV");
        arg("display", display);
        arg("surface", surface);
        arg("x", x);
        arg("y", y);
        ...
        boolean result = mEgl10.eglPostSubBufferNV(display, surface, x, y, width, height);
        returns(result);
        checkError();
        return result;
    }
