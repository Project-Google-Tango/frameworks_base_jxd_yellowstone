...
status_t NativeInputEventReceiver::consumeEvents(JNIEnv* env,
    ...
    for (;;) {
        ...
        // Skip dispatching events if too many events are forwarded to ANR dialog.
        // System server can crash due to the bunch of pending events generated
        // while an application is in ANR status.
        if (!skipCallbacks && nevents++ > 100) {
            if (strstr(getInputChannelName(), "Application Not Responding")) {
                ALOGE("channel '%s' ~ Skip dispatching remaining events to avoid system server crashes",
                        getInputChannelName());
                skipCallbacks = true;
            }
        }
