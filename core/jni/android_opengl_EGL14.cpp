...
/* EGLBoolean eglPostSubBufferNV ( EGLDisplay dpy, EGLSurface surface, int x, int y, int width, int height ) */
static jboolean
android_eglPostSubBufferNV
  (JNIEnv *_env, jobject _this, jobject dpy, jobject surface, int x, int y, int width, int height) {
    EGLBoolean _returnValue = (EGLBoolean) 0;
    EGLDisplay dpy_native = (EGLDisplay) fromEGLHandle(_env, egldisplayGetHandleID, dpy);
    EGLSurface surface_native = (EGLSurface) fromEGLHandle(_env, eglsurfaceGetHandleID, surface);

    return eglPostSubBufferNV(dpy_native, surface_native, x, y, width, height);
}
...
{"eglPostSubBufferNV", "(Landroid/opengl/EGLDisplay;Landroid/opengl/EGLSurface;IIII)Z", (void *) android_eglPostSubBufferNV },
};
