...
static jboolean jni_eglPostSubBufferNV(JNIEnv *_env, jobject _this, jobject display, jobject surface, jint x, jint y, jint width, jint height) {
    if (display == NULL || surface == NULL) {
        jniThrowException(_env, "java/lang/IllegalArgumentException", NULL);
        return JNI_FALSE;
    }
    ...
    return eglPostSubBufferNV(dpy, sur, x, y, width, height);
}
...
{"eglPostSubBufferNV",     "(" DISPLAY SURFACE "IIII)Z", (void*)jni_eglPostSubBufferNV },
};
