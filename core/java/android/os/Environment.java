...
public class Environment {
    ...
    private static StorageVolume getSecondaryVolume() {
        // Do not cache since it is removable
        StorageVolume sSecondaryVolume = null;
        try {
            IMountService mountService = IMountService.Stub.asInterface(ServiceManager
                    .getService("mount"));
            final StorageVolume[] volumes = mountService.getVolumeList();
            sSecondaryVolume = StorageManager.getSecondaryVolume(volumes);
        } catch (Exception e) {
            Log.e(TAG, "couldn't talk to MountService", e);
        }
    ...
    public static class UserEnvironment {
        ...
        /** Secondary external storage dir */
        private File mSecondaryDir = null;
        private final File mSecondaryStorageAndroidObb;
        ...
        public UserEnvironment(int userId) {
            ...
            if (!TextUtils.isEmpty(rawSecondaryStorage) && userId == UserHandle.USER_OWNER) {
                for (String secondaryPath : rawSecondaryStorage.split(":")) {
                    ...
                    // use the first entry for moving apps
                    if (mSecondaryDir == null) {
                        mSecondaryDir = new File(secondaryPath);
                    }
                }
            }
            ...
            mSecondaryStorageAndroidObb = buildPath(mSecondaryDir, DIRECTORY_ANDROID, "obb");
        }
        ...
        public File getSecondaryStorageDirectory() {
            return mSecondaryDir;
        }

        public File getSecondaryStorageObbDirectory() {
            return mSecondaryStorageAndroidObb;
        }

        public File getSecondaryStorageAppObbDirectory(String packageName) {
            return new File(mSecondaryStorageAndroidObb, packageName);
        }
    }
    ...
    /** {@hide} */
    public static File getSecondaryStorageObbDirectory() {
        return buildPath(getSecondaryStorageDirectory(), DIRECTORY_ANDROID, "obb");
    }
    ...
    /**
     * Generates the raw path to an application's secondary storage OBB files
     * @hide
     */
    public static File getSecondaryStorageAppObbDirectory(String packageName) {
        throwIfUserRequired();
        return sCurrentUser.getSecondaryStorageAppObbDirectory(packageName);
    }
    ...
    public static String getSecondaryStorageState() {
        try {
            IMountService mountService = IMountService.Stub.asInterface(ServiceManager
                    .getService("mount"));
            final StorageVolume secondary = getSecondaryVolume();
            String rv = mountService.getVolumeState(secondary.getPath());
            return rv;
