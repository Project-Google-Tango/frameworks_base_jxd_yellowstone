...
public class StorageManager {
    ...
    public StorageVolume getSecondaryVolume() {
        return getSecondaryVolume(getVolumeList());
    }

    /** {@hide} */
    public static StorageVolume getSecondaryVolume(StorageVolume[] volumes) {
        for (StorageVolume volume : volumes) {
            if (!volume.isPrimary() && volume.isRemovable() && !volume.isEmulated()) {
                return volume;
            }
        }
        Log.w(TAG, "No secondary storage defined");
        return null;
    }
}
