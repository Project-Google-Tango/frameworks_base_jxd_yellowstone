...
//                setRingerIcon();
    ...
    public void onProgressChanged(SeekBar seekBar, int progress,
        ...
        if (fromUser && tag instanceof StreamControl) {
            ...
            progress =  NvEdpManager.checkDeviceLimit(NvEdpManager.EDP_CLIENT_AUDIO_VOLUME, progress, getStreamMaxVolume(sc.streamType));
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        ...
        if (tag instanceof StreamControl) {
            ...
            if (sc.streamType == AudioService.STREAM_REMOTE_MUSIC) {
                ...
                /* NvEdpManager Note:
                 * seekBar.setProgress doesn't work in onProgressChanged() because VolumePanel is not running on UiThread,
                 * and ProgressBar refreshProgress() treats UiTread and non-UiThread differently,
                 * for non-UiThread, if the next refresh comes before previous one is done, the next refresh gets ignored
                 * so seekBar.setProgress in onProgressChanged() is ignored because ProgressBar is still refreshing for touch input from user.
                 * setProgress here gets a better and un-jumpy result.
                 */
                seekBar.setProgress(getStreamVolume(sc.streamType));
            }
        }
    }
