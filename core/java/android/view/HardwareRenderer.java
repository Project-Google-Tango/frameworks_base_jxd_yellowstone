...
public abstract class HardwareRenderer {
    ...
    private boolean mUsePostSubBuffer = false;
     ...
     */
    static HardwareRenderer createGlRenderer(int glVersion, boolean translucent, boolean useFramebufferAge) {
        switch (glVersion) {
            case 2:
                return Gl20Renderer.create(translucent, useFramebufferAge);
        }
    ...
    /**
     * Indicates the view would like the renderer to use eglPostSubBufferNV rather than eglSwapBuffers
     */
    void setUsePostSubBuffer(boolean usePostSubBuffer) {
        mUsePostSubBuffer = usePostSubBuffer;
        if (usePostSubBuffer) {
            Log.v(LOG_TAG, "Using PostSubBuffer");
        }
    }
    boolean usePostSubBuffer() {
        return mUsePostSubBuffer;
    }
    ...
    static abstract class GlRenderer extends HardwareRenderer {
        // EGL_EXT_buffer_age is not exposed in our EGL APIs. 
        ...
        static boolean sHas_EGL_EXT_buffer_age;
        ...
        private boolean mUseFramebufferAge;

        private long mDrawDelta = Long.MAX_VALUE;

        private static final int DIRTY_HISTORY_SIZE = 4;
        private class DirtyHistory {
            private Rect[] mQueue = new Rect[DIRTY_HISTORY_SIZE];
            private int mBack = DIRTY_HISTORY_SIZE - 1;
            public Rect get(int index) {
                if (index >= DIRTY_HISTORY_SIZE || index < 0)
                    return null;
                return mQueue[(1 + mBack + index) % DIRTY_HISTORY_SIZE];
            }
            public void prepend(Rect rect) {
                mQueue[mBack] = (rect != null) ? new Rect(rect) : null;
                mBack = (mBack + DIRTY_HISTORY_SIZE - 1) % DIRTY_HISTORY_SIZE;
            }
            public Rect unionWith(Rect rect, int count) {
                if (rect == null || count > DIRTY_HISTORY_SIZE || count < 0)
                    return null;
                Rect union = new Rect(rect);
        ...
        private DirtyHistory mDirtyHistory = new DirtyHistory();

        GlRenderer(int glVersion, boolean translucent, boolean useFramebufferAge) {
            initializeEgl();
            ...
            mUseFramebufferAge = sDirtyRegions && useFramebufferAge && sHas_EGL_EXT_buffer_age;
        ...
        boolean initialize(Surface surface) throws OutOfResourcesException {
            if (isRequested() && !isEnabled()) {
                ...
                int[] configSpec = getConfig(!mUseFramebufferAge && sDirtyRegions);
                mEglConfig = chooseEglConfig(configSpec);
                if (mEglConfig == null)
                   throw new RuntimeException("Failed to choose an appropriate EGLConfig.");

                ManagedEGLContext managedContext = sEglContextStorage.get();
                mEglContext = managedContext != null ? managedContext.getContext() : null;

                if (mEglContext == null) {
                    mEglContext = createContext(sEgl, sEglDisplay, mEglConfig);
        ...
        abstract int[] getConfig(boolean preserveBackBuffer);

        static boolean initializeEgl() {
            synchronized (sEglLock) {
                if (sEgl == null) {
                    ...
                    // Check for EGL_EXT_buffer_age.
                    for (String extension : sEgl.eglQueryString(sEglDisplay, EGL_EXTENSIONS).split(" ")) {
                        if (extension.equals("EGL_EXT_buffer_age")) {
                            sHas_EGL_EXT_buffer_age = true;
                            break;
                        }
                    }
                }
        ...
        protected static EGLConfig chooseEglConfig(int[] configSpec) {
        ...
        private void enableDirtyRegions() {
            if (mUseFramebufferAge) {
                // If mUseFramebufferAge is set, our EGL config does not
                // have EGL_SWAP_BEHAVIOR_PRESERVED_BIT. The swap behavior
                // still might be EGL_BUFFER_PRESERVED though if it's the only
                // mode supported by the driver, and in that case we don't need
                // to use buffer age.
                mDirtyRegionsEnabled = true;
                mUseFramebufferAge = !isBackBufferPreserved();
            } else if (sDirtyRegions) {
                // If sDirtyRegions is set and mUseFramebufferAge is false, this
                // means we have an EGL config with EGL_SWAP_BEHAVIOR_PRESERVED_BIT.
        ...
        private boolean createSurface(Surface surface) {
            mEglSurface = sEgl.eglCreateWindowSurface(sEglDisplay, mEglConfig, surface, null);
        ...
        void draw(View view, View.AttachInfo attachInfo, HardwareDrawCallbacks callbacks,
                Rect dirty) {
            if (canDraw()) {
                if (!hasDirtyRegions() && !usePostSubBuffer()) {
                    dirty = null;
                }
        ...
        private int prepareFrame(Rect dirty) {
            int status;
            Rect patchedDirtyRect = dirty;
            if (mUseFramebufferAge) {
                int[] framebufferAge = new int[1];
                sEgl.eglQuerySurface(sEglDisplay, mEglSurface, EGL_BUFFER_AGE_EXT, framebufferAge);
                // 'unionWith' returns null if count < 0.
                patchedDirtyRect = mDirtyHistory.unionWith(patchedDirtyRect, framebufferAge[0] - 1);
            }
            Trace.traceBegin(Trace.TRACE_TAG_VIEW, "prepareFrame");
            try {
                status = onPreDraw(patchedDirtyRect);
            } finally {
                Trace.traceEnd(Trace.TRACE_TAG_VIEW);
            }
            return status;
        ...
        private void swapBuffers(int status, Rect dirty) {
            if ((status & DisplayList.STATUS_DREW) == DisplayList.STATUS_DREW) {
                ...
                if (usePostSubBuffer()) {
                    if (dirty == null) {
                        sEgl.eglPostSubBufferNV(sEglDisplay, mEglSurface, 0, 0, mWidth, mHeight);
                    } else {
                        sEgl.eglPostSubBufferNV(sEglDisplay, mEglSurface, dirty.left, mHeight - dirty.bottom, dirty.width(), dirty.height());
                    }
                } else {
                    sEgl.eglSwapBuffers(sEglDisplay, mEglSurface);
                }
                if (mUseFramebufferAge)
                    mDirtyHistory.prepend(dirty);
    ...
    static class Gl20Renderer extends GlRenderer {
        ...
        Gl20Renderer(boolean translucent, boolean useFramebufferAge) {
            super(2, translucent, useFramebufferAge);
        }
        ...
        @Override
        int[] getConfig(boolean preserveBackBuffer) {
            return getConfigSpec(preserveBackBuffer);
        }
        ...
        static HardwareRenderer create(boolean translucent, boolean useFramebufferAge) {
            if (GLES20Canvas.isAvailable()) {
                return new Gl20Renderer(translucent, useFramebufferAge);
            }
            return null;
        }

        private static int[] getConfigSpec(boolean preserveBackBuffer) {
            //noinspection PointlessBooleanExpression,ConstantConditions
            final int stencilSize = GLES20Canvas.getStencilSize();
            final int swapBehavior = preserveBackBuffer ? EGL14.EGL_SWAP_BEHAVIOR_PRESERVED_BIT : 0;
            return new int[] {
        ...
        private static void usePbufferSurface(EGLContext eglContext) {
            synchronized (sPbufferLock) {
                ...
                if (sPbuffer == null) {
                    int[] configSpec = getConfigSpec(sDirtyRegions);
                    EGLConfig config = chooseEglConfig(configSpec);
                    sPbuffer = sEgl.eglCreatePbufferSurface(sEglDisplay, config, new int[] {
