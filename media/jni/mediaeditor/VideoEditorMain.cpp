...
#ifdef PROFILING
                pContext->mClipDuration = pContext->pEditSettings->pClipList[i]->ClipProperties.uiClipDuration;
#endif
            } else {
                pEnv->DeleteLocalRef(properties);
                goto videoEditor_populateSettings_cleanup;
...
videoEditor_generateClip(
    ...
    int64_t            startTime;
    int64_t            endTime;
    SystemTimeSource   sysTime;
    int                profFlag=0;
    char               value[PROPERTY_VALUE_MAX];

    property_get("enable-VEkpi", value, NULL);
    profFlag = atoi(value);
#endif
...
#ifdef PROFILING
    if (profFlag)
        startTime = sysTime.getRealTimeUs();
#endif
    result = videoEditor_processClip(pEnv, thiz, 0 /*item id is unused*/);
#ifdef PROFILING
    if (profFlag)
    {
        endTime = sysTime.getRealTimeUs();
        ALOGI("-----Clip Duration = %.2f sec",(pContext->mClipDuration / 1000.0));
        ALOGI("-----Total Export Time =  %.2f sec",((endTime- startTime) / 1E6));
    }
#endif
