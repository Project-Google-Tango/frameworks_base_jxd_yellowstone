...
android_media_MediaProfiles_native_get_camcorder_profile(JNIEnv *env, jobject thiz, jint id, jint quality)
{
    ...
    int videoProfile     = sProfiles->getCamcorderProfileParamByName("vid.profile", id, q);
    int numBFrames       = sProfiles->getCamcorderProfileParamByName("vid.Bframes", id, q);
    ...
    if (duration == -1 || fileFormat == -1 || videoCodec == -1 || audioCodec == -1 ||
        videoBitRate == -1 || videoFrameRate == -1 || videoFrameWidth == -1 ||
        videoFrameHeight == -1 || videoProfile == -1 || numBFrames == -1 ||
    ...
    jmethodID camcorderProfileConstructorMethodID = env->GetMethodID(camcorderProfileClazz, "<init>", "(IIIIIIIIIIIIII)V");
