...
public class MediaRecorder
{
    ...
    public void setProfile(CamcorderProfile profile) {
        ...
        setVideoEncoderProfile(profile.videoProfile);
        setVideoEncoderNumBFrames(profile.numBFrames);
    ...
    /**
     * Set Super Fine Quality for Video. This can be used to turn on Temporal Noise reduction algorithm
     * on input video frames.
     * The algorithm is implemented on 3D Shaders and removes noise introduced in low light capture.
     * The algorithm adapts to various lighting conditions.
     * No other parameters are required to be set here.
     */
    public void setSuperFineQuality() {
        setParameter("video-super-fine-quality-enable=1");
    }
     ...
     * Prepare() may perform additional checks on the parameter to make sure whether the
     * specified profile is applicable.
     *
     * @param profile the video encoding profile.
     */
    public void setVideoEncoderProfile(int profile) {
        if (profile <= 0) {
            throw new IllegalArgumentException("Video encoder profile is not positive");
        }
        setParameter("video-param-encoder-profile=" + profile);
    }

    /**
     * Sets the number of B frames to be encoded between 2 I/P for recording.
     * Call this method before prepare().
     * Prepare() may perform additional checks on the parameter to make sure whether the
     * specified number of B frames is applicable.
     *
     * @param numBFrames number of B frames between 2 I/P frames.
     */
    public void setVideoEncoderNumBFrames(int numBFrames) {
        if (numBFrames < 0) {
            throw new IllegalArgumentException("number of B frames is not positive");
        }
        setParameter("video-param-encoder-numBFrames=" + numBFrames);
    }
