...
// Log debug message about the pressure smoothing
#define DEBUG_PRESSURE_SMOOTHING 0

// Log debug message about the filter out
#define DEBUG_FILTER_OUT 0

// Log debug message about the tool switch tracking
#define DEBUG_TOOL_SWITCH 0

#if (DEBUG_FILTER_OUT || DEBUG_TOOL_SWITCH)
#define GET_TOOLTYPE_STR(type) \
        type == AMOTION_EVENT_TOOL_TYPE_FINGER ? "FINGER" : \
        type == AMOTION_EVENT_TOOL_TYPE_ERASER ? "ERASER" : \
        type == AMOTION_EVENT_TOOL_TYPE_STYLUS ? "PEN" : \
        type == AMOTION_EVENT_TOOL_TYPE_MOUSE ? "MOUSE" : "UNKNOWN"
#endif
...
static inline bool isTimeOut(nsecs_t now, nsecs_t last, nsecs_t time) {
    return ((now - last) > time);
}

static inline bool isRangeOut(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t range) {
    return (((x1 - range) > x2) || ((x1 + range) < x2) ||
            ((y1 - range) > y2) || ((y1 + range) < y2));
}
...
void InputReader::updateLastBlackoutTimeLocked(nsecs_t time) {
    mLastBlackoutTime = time;
}

nsecs_t InputReader::getLastBlackoutTimeLocked() {
    return mLastBlackoutTime;
}
...
void InputReader::ContextImpl::updateLastBlackoutTime(nsecs_t time) {
    mReader->updateLastBlackoutTimeLocked(time);
}

nsecs_t InputReader::ContextImpl::getLastBlackoutTime() {
    return mReader->getLastBlackoutTimeLocked();
}
...
CursorInputMapper::CursorInputMapper(InputDevice* device) :
        InputMapper(device) {
    InputFilter::RSMouse::registerCursorInputMapper(this, device);
    mHideTouch = false;
}

CursorInputMapper::~CursorInputMapper() {
    InputFilter::RSMouse::registerCursorInputMapper(NULL, getDevice());
}
...
void CursorInputMapper::configure(nsecs_t when,
    ...
    if (!changes) { // first time only
        ...
        InputFilter::RSMouse::registerPointerController(mPointerController);
        mVWheelScale = 1.0f;
        mHWheelScale = 1.0f;
    ...
    case 0:
        ...
        break;
    case 1:
        mParameters.mode = Parameters::MODE_TOUCH;
        break;
    }
    ...
    case Parameters::MODE_POINTER:
        ...
        mXScale = 1.0f;
        mYScale = 1.0f;
        mPresentation = PointerControllerInterface::PRESENTATION_POINTER;
        break;
...
void CursorInputMapper::sync(nsecs_t when) {
    ...
    if (mPointerController != NULL) {
        if (moved || scrolled || buttonsChanged) {
            mPointerController->setPresentation(mPresentation);
...
void CursorInputMapper::fadePointer() {
    if (mPointerController != NULL) {
        InputFilter::RSMouse::notifyCursorPointerFade();
...
TouchInputMapper::TouchInputMapper(InputDevice* device) :
        ...
        mTouchBlackoutWindow(s2ns(3)),
        mIIRLowPassFactor(1.0f),
        mPressureEBC(0.0f),
        mPointersDownSinceStylus(false),
        mExtendBlackoutWhilePointersDown(true),
        mTouchBlackoutEnabled(true),
        mTouchBlackoutConfiguredFromSettings(false),
        mTouchLocked(false),
        mFingerOnlyMode(true),
        mFirstToolId(0),
        mStylusLockOnEdgeMarginLeft(0),
        mStylusLockOnEdgeMarginTop(0),
        mStylusLockOnEdgeMarginRight(0),
        mStylusLockOnEdgeMarginBottom(0),
        mStylusWhitelistedApp(false),
        mStylusLeftHandedMode(false),
        mIdRawLastPointers(0),
        mIdBlockedPointers(0),
        mIdFinger(0), mIdPen(0),mIdEraser(0),
        mToolSwitchTime(0),
        mToolSwitchRange(0),
        mEraserLatchRange(0),
        mEraserDelayTime(0),
        mIdLastValidPointers(0),
        mIdTrackingPointers(0),
...
void TouchInputMapper::populateDeviceInfo(InputDeviceInfo* info) {
    ...
    if (mDeviceMode != DEVICE_MODE_DISABLED) {
        ...
        if (!mTouchBlackoutConfiguredFromSettings) {
            property_get("input.tch_blk.enabled", buff, "");
            if (*buff) {
                int touchBlackoutEnabled = 0;
                sscanf(buff, "%d", &touchBlackoutEnabled);
                mTouchBlackoutEnabled = !!touchBlackoutEnabled;
                ALOGI("Setting mTouchBlackoutEnabled from property to %d", mTouchBlackoutEnabled);
            }

            property_get("input.tch_blk.timeout_ms", buff, "");
            if (*buff) {
                nsecs_t touchBlackoutWindowMs = 0;
                sscanf(buff, "%lld", &touchBlackoutWindowMs);
                mTouchBlackoutWindow = 1000ll*1000ll*touchBlackoutWindowMs;
            }
        }

        // This isn't read from settings
        property_get("input.tch_blk.ext_for_ptrs", buff, "");
        if (*buff) {
            int extendBlackoutWhilePointersDown = 0;
            sscanf(buff, "%d", &extendBlackoutWhilePointersDown);
            mExtendBlackoutWhilePointersDown = !!extendBlackoutWhilePointersDown;
            ALOGI("Setting mExtendBlackoutWhilePointersDown from property to %d",
                  mExtendBlackoutWhilePointersDown);
        }

        property_get("input.tch_prs.iir", buff, "");
        if (*buff) {
            sscanf(buff, "%f", &mIIRLowPassFactor);
        }
        if (mIIRLowPassFactor > 1.0f)
            mIIRLowPassFactor = 1.0f;
        else if (mIIRLowPassFactor < 0.000001f)
            mIIRLowPassFactor = 0.000001f;
        ALOGI("Setting mIIRLowPassFactor from property to %f", mIIRLowPassFactor);

        property_get("input.tch_prs.ebc", buff, "");
        if (*buff) {
            sscanf(buff, "%f", &mPressureEBC);
        }
        if (mPressureEBC > 1.0f)
            mPressureEBC = 1.0f;
        else if (mPressureEBC < 0.0f)
            mPressureEBC = 0.0f;
        ALOGI("Setting mPressureEBC from property to %f", mPressureEBC);

        property_get("input.stl_blk.edge_left", buff, "");
        if (*buff) {
            sscanf(buff, "%d", &mStylusLockOnEdgeMarginLeft);
            ALOGI("Setting mStylusLockOnEdgeMarginLeft from property to %d", mStylusLockOnEdgeMarginLeft);
        }
        property_get("input.stl_blk.edge_top", buff, "");
        if (*buff) {
            sscanf(buff, "%d", &mStylusLockOnEdgeMarginTop);
            ALOGI("Setting mStylusLockOnEdgeMarginTop from property to %d", mStylusLockOnEdgeMarginTop);
        }
        property_get("input.stl_blk.edge_right", buff, "");
        if (*buff) {
            sscanf(buff, "%d", &mStylusLockOnEdgeMarginRight);
            ALOGI("Setting mStylusLockOnEdgeMarginRight from property to %d", mStylusLockOnEdgeMarginRight);
        }
        property_get("input.stl_blk.edge_bottom", buff, "");
        if (*buff) {
            sscanf(buff, "%d", &mStylusLockOnEdgeMarginBottom);
            ALOGI("Setting mStylusLockOnEdgeMarginBottom from property to %d", mStylusLockOnEdgeMarginBottom);
        }

        // Parameters for pen and eraser tracking and lock.
        property_get("input.ptr_trk.toolswitch_time", buff, "");
        if (*buff) {
            nsecs_t toolSwitchTimeMs = 0;
            sscanf(buff, "%lld", &toolSwitchTimeMs);
            mToolSwitchTime = 1000ll*1000ll*toolSwitchTimeMs;
            ALOGI("Setting mToolSwitchTime from property to %lld ms", toolSwitchTimeMs);
        }
        property_get("input.ptr_trk.toolswitch_range", buff, "");
        if (*buff) {
            sscanf(buff, "%d", &mToolSwitchRange);
            ALOGI("Setting mToolSwitchRange from property to %d", mToolSwitchRange);
        }
        property_get("input.ptr_trk.eraserlatch_range", buff, "");
        if (*buff) {
            sscanf(buff, "%d", &mEraserLatchRange);
            ALOGI("Setting mEraserLatchRange from property to %d", mEraserLatchRange);
        }
        property_get("input.ptr_trk.eraserdelay_time", buff, "");
        if (*buff) {
            nsecs_t eraserDelayTimeMs = 0;
            sscanf(buff, "%lld", &eraserDelayTimeMs);
            mEraserDelayTime = 1000ll*1000ll*eraserDelayTimeMs;
            ALOGI("Setting mEraserDelayTime from property to %lld ms", eraserDelayTimeMs);
        }
    }
}
...
void TouchInputMapper::dump(String8& dump) {
    ...
    dump.appendFormat(INDENT3 "TouchBlackoutWindowNs: %lld\n", mTouchBlackoutWindow);
    dump.appendFormat(INDENT3 "ExtendBlackoutWhilePointersDown: %d\n", mExtendBlackoutWhilePointersDown);
    dump.appendFormat(INDENT3 "TouchBlackoutEnabled: %d\n", mTouchBlackoutEnabled);
    dump.appendFormat(INDENT3 "TouchBlackoutConfiguredFromSettings: %d\n", mTouchBlackoutConfiguredFromSettings);
    dump.appendFormat(INDENT3 "TouchLocked: %d\n", mTouchLocked);
...
void TouchInputMapper::configure(nsecs_t when,
    ...
    if (!changes || (changes & InputReaderConfiguration::CHANGE_BLACKOUT)) {
        if (config->penToTouchDelayMs >= 0) {
            mTouchBlackoutEnabled = (config->penToTouchDelayMs > 0);
            mTouchBlackoutWindow = config->penToTouchDelayMs*1000LL*1000LL;
            mTouchBlackoutConfiguredFromSettings = true;
            ALOGI("mTouchBlackoutEnabled after change from settings: %d", mTouchBlackoutEnabled);
        } else {
            mTouchBlackoutConfiguredFromSettings = false;
        }

        mTouchLocked = config->lockTouch;
        ALOGI("mTouchLocked after change from request: %d", mTouchLocked);
    }

    if (!changes || (changes & InputReaderConfiguration::CHANGE_FINGER_ONLY_MODE)) {
        mFingerOnlyMode = config->fingerOnlyMode;
        ALOGI("mFingerOnlyMode after change from request: %d", mFingerOnlyMode);
    }

    if (!changes || (changes & InputReaderConfiguration::CHANGE_STYLUS_WHITELISTED_APP)) {
        mStylusWhitelistedApp = config->stylusWhitelistedApp;
        ALOGI("mStylusWhitelistedApp after change from request: %d", mStylusWhitelistedApp);
    }

    if (!changes || (changes & InputReaderConfiguration::CHANGE_LEFT_HANDED_MODE)) {
        mStylusLeftHandedMode = config->leftHandedMode;
        ALOGI("mStylusLeftHandedMode after change from request: %d", mStylusLeftHandedMode);
    }

    bool resetNeeded = false;
    if (!changes || (changes & (InputReaderConfiguration::CHANGE_DISPLAY_INFO
            | InputReaderConfiguration::CHANGE_POINTER_GESTURE_ENABLEMENT
            | InputReaderConfiguration::CHANGE_STYLUS_WHITELISTED_APP
...
void TouchInputMapper::configureSurface(nsecs_t when, bool* outResetNeeded) {
    ...
    if (mParameters.deviceType == Parameters::DEVICE_TYPE_POINTER
        ...
        if (mStylusWhitelistedApp && hasStylus()) {
            mSource |= AINPUT_SOURCE_STYLUS;
        }
    ...
    if (mDeviceMode == DEVICE_MODE_POINTER ||
            (mDeviceMode == DEVICE_MODE_DIRECT &&
             (mConfig.showTouches || mConfig.showStylus || mConfig.showEraser))) {
        if (mPointerController == NULL) {
            mPointerController = getPolicy()->obtainPointerController(getDeviceId());
        }
    } else {
...
void TouchInputMapper::updateBlackout(nsecs_t when) {
    // Refresh blackout if we are not in finger only mode and
    // if we want to extend the blackout and if there is any touch and
    // if the pointers we only up for the blackout duration,
    // even if the pointers are just fingers
    if (!mFingerOnlyMode && mExtendBlackoutWhilePointersDown &&
        (mCurrentRawPointerData.touchingIdBits.value |
        mCurrentRawPointerData.hoveringIdBits.value) &&
        (when < mContext->getLastBlackoutTime() + mTouchBlackoutWindow)) {
        mPointersDownSinceStylus = true;
    }
    else {
        mPointersDownSinceStylus = false;
    }

    // If we are not in finger only mode, check if there is any PEN
    // tool type in current pointers
    if (!mPointersDownSinceStylus && !mFingerOnlyMode &&
        ((mCurrentRawPointerData.touchingIdBits.value |
        mCurrentRawPointerData.hoveringIdBits.value) & mIdPen.value)) {
            mPointersDownSinceStylus = true;
    }

    if (mPointersDownSinceStylus) {
        mContext->updateLastBlackoutTime(when);
    }
}

void TouchInputMapper::blockPointersForBlackout(nsecs_t when) {
    // Don't do this if we are in finger only mode or if blackout is disabled
    // or if we are outside a blackout window.
    // We need to do this if touch is locked regardless of blackout status
    if (mFingerOnlyMode ||
        (!mTouchLocked && (!mTouchBlackoutEnabled ||
        (when > (mContext->getLastBlackoutTime() + mTouchBlackoutWindow))))) {
        return;
    }
    ...
    for (BitSet32 idBits((mCurrentRawPointerData.touchingIdBits.value |
         ...
         !idBits.isEmpty(); ) {
        uint32_t id = idBits.clearFirstMarkedBit();
        mCurrentRawPointerData.clearIdBit(id);
    }
}

void TouchInputMapper::filterOutSubPointers() {
    // Do not apply this filter when finger only mode is enabled.
    if (mFingerOnlyMode)
        return;

    BitSet32 idCurrentPointers(mCurrentRawPointerData.touchingIdBits.value |
                               mCurrentRawPointerData.hoveringIdBits.value);
    ...
    for (BitSet32 idBits(mIdTrackingPointers); !idBits.isEmpty(); ) {
        uint32_t id = idBits.clearFirstMarkedBit();
        int32_t tooltype = idCurrentPointers.hasBit(id) ?
                           mCurrentRawPointerData.pointerForId(id).toolType :
                           mPointerTrack[id].toolType;

        if (tooltype == AMOTION_EVENT_TOOL_TYPE_STYLUS) {
            mIdPen.markBit(id);
        }
        else if (tooltype == AMOTION_EVENT_TOOL_TYPE_ERASER) {
            mIdEraser.markBit(id);
        }
        else if (tooltype == AMOTION_EVENT_TOOL_TYPE_FINGER) {
            mIdFinger.markBit(id);
        }
    }

    if (mIdPen.isEmpty() && mIdEraser.isEmpty())
        return;

    // Pen is higher priority than eraser,
    // so keep previous ID if corresponding pen exists.
    // If not, find pen first if there is any other pen pointer.
    // And then, check eraser.
    if (!mIdPen.hasBit(mFirstToolId)) {
        if (!mIdPen.isEmpty()) {
            mFirstToolId = mIdPen.firstMarkedBit();
        }
        else if (!mIdEraser.hasBit(mFirstToolId) && !mIdEraser.isEmpty()) {
            mFirstToolId = mIdEraser.firstMarkedBit();
        }
    }

#if DEBUG_FILTER_OUT
    ALOGD("filterOut: finding first tool - %d = %s", mFirstToolId,
        GET_TOOLTYPE_STR(mCurrentRawPointerData.pointerForId(mFirstToolId).toolType));
#endif

    // Remove all inputs except first.
    for (BitSet32 idBits(idCurrentPointers.value &
                         (mIdPen.value | mIdEraser.value));
        !idBits.isEmpty(); ) {
        uint32_t id = idBits.clearFirstMarkedBit();

        if (id != mFirstToolId) {
#if DEBUG_FILTER_OUT
            ALOGD("filterOut: pointer id %d (%d), type %s - Not first input",
                id, mCurrentRawPointerData.idToIndex[id],
                GET_TOOLTYPE_STR(mCurrentRawPointerData.pointerForId(id).toolType));
#endif
            mCurrentRawPointerData.clearIdBit(id);
        }
    }
...
void TouchInputMapper::filterOutEdgePointers() {
    // We apply this filter only when whitelisted app is in foreground
    // and when it is enabled with valid pixel margin values.
    if (!mStylusWhitelistedApp ||
        !(mStylusLockOnEdgeMarginLeft > 0 || mStylusLockOnEdgeMarginTop > 0 ||
        mStylusLockOnEdgeMarginRight > 0 || mStylusLockOnEdgeMarginBottom > 0))
        return;

    BitSet32 idCurrentPointers(mCurrentRawPointerData.touchingIdBits.value |
                               mCurrentRawPointerData.hoveringIdBits.value);
    BitSet32 idUpPointers(mIdRawLastPointers.value & ~idCurrentPointers.value);
    BitSet32 idDownPointers(idCurrentPointers.value & ~mIdRawLastPointers.value);

    mIdRawLastPointers = idCurrentPointers;
    mIdBlockedPointers.value &= ~idUpPointers.value;

    for (BitSet32 idBits(idCurrentPointers); !idBits.isEmpty(); ) {
        uint32_t id = idBits.clearFirstMarkedBit();

        if (idDownPointers.hasBit(id)) {
            int32_t toolType = mCurrentRawPointerData.pointerForId(id).toolType;
            int32_t px = mCurrentRawPointerData.pointerForId(id).x;
            int32_t py = mCurrentRawPointerData.pointerForId(id).y;
            int32_t x, y, maxX, maxY;

            switch (mSurfaceOrientation) {
            case DISPLAY_ORIENTATION_90:
                x = py - mRawPointerAxes.y.minValue;
                y = mRawPointerAxes.x.maxValue - px;
                maxX = mRawPointerAxes.y.maxValue - mRawPointerAxes.y.minValue;
                maxY = mRawPointerAxes.x.maxValue - mRawPointerAxes.x.minValue;
                break;
            case DISPLAY_ORIENTATION_180:
                x = mRawPointerAxes.x.maxValue - px;
                y = mRawPointerAxes.y.maxValue - py;
                maxX = mRawPointerAxes.x.maxValue - mRawPointerAxes.x.minValue;
                maxY = mRawPointerAxes.y.maxValue - mRawPointerAxes.y.minValue;
                break;
            case DISPLAY_ORIENTATION_270:
                x = mRawPointerAxes.y.maxValue - py;
                y = px - mRawPointerAxes.x.minValue;
                maxX = mRawPointerAxes.y.maxValue - mRawPointerAxes.y.minValue;
                maxY = mRawPointerAxes.x.maxValue - mRawPointerAxes.x.minValue;
                break;
            default:
                x = px - mRawPointerAxes.x.minValue;
                y = py - mRawPointerAxes.y.minValue;
                maxX = mRawPointerAxes.x.maxValue - mRawPointerAxes.x.minValue;
                maxY = mRawPointerAxes.y.maxValue - mRawPointerAxes.y.minValue;
                break;
            }
            ...
            if ((toolType == AMOTION_EVENT_TOOL_TYPE_STYLUS ||
                ...
                ((mStylusLeftHandedMode && mStylusLockOnEdgeMarginLeft > 0 &&
                    x < mStylusLockOnEdgeMarginLeft) ||
                (mStylusLockOnEdgeMarginTop > 0 && y < mStylusLockOnEdgeMarginTop) ||
                (!mStylusLeftHandedMode && mStylusLockOnEdgeMarginRight > 0 &&
                    x > (maxX - mStylusLockOnEdgeMarginRight)) ||
                (mStylusLockOnEdgeMarginBottom > 0 && y > (maxY - mStylusLockOnEdgeMarginBottom)))) {
#if DEBUG_FILTER_OUT
                ALOGD("filterOut: pointer id %d (%d), type %s - Edge started",
                        id, mCurrentRawPointerData.idToIndex[id],
                        GET_TOOLTYPE_STR(toolType));
                ALOGD("           raw (%d,%d) => trans (%d,%d), max (%d,%d)",
                        px, py, x, y, maxX, maxY);
#endif
                mIdBlockedPointers.markBit(id);
            }
        }

        if (mIdBlockedPointers.hasBit(id)) {
#if DEBUG_FILTER_OUT
            ALOGD("filterOut: pointer id %d (%d), type %s - Edge input",
                    id, mCurrentRawPointerData.idToIndex[id],
                    GET_TOOLTYPE_STR(mCurrentRawPointerData.pointerForId(id).toolType));
#endif
            mCurrentRawPointerData.clearIdBit(id);
...
void TouchInputMapper::trackToolSwitch(nsecs_t when) {
    // Don't need to do this if finger only mode is enabled.
    if (mFingerOnlyMode)
        return;
    ...
    if (!(mToolSwitchTime || mToolSwitchRange ||
        mEraserLatchRange || mEraserDelayTime)) {
#if DEBUG_TOOL_SWITCH
        ALOGD("toolSwitch: disabled, just return");
#endif
        mIdTrackingPointers = idCurrentPointers;
        return;
    }

    mIdTrackingPointers.value |= idCurrentPointers.value;

    BitSet32 idUpPointers(mIdLastValidPointers.value & ~idCurrentPointers.value);
    BitSet32 idDownPointers(idCurrentPointers.value & ~mIdLastValidPointers.value);

    for (BitSet32 idBits(mIdTrackingPointers); !idBits.isEmpty(); ) {
        uint32_t id = idBits.clearFirstMarkedBit();

        bool isSwitchTimeout = isTimeOut(when, mPointerTrack[id].lastUpTime, mToolSwitchTime);
        bool isEraserTimeout = isTimeOut(when, mPointerTrack[id].lastDownTime, mEraserDelayTime);

        int32_t px = 0;
        int32_t py = 0;
        int32_t tooltype = 0;
        bool isToolSwitchRangeOut = true;
        bool isEraserLatchRangeOut = true;

        if (idCurrentPointers.hasBit(id)) {
            tooltype = mCurrentRawPointerData.pointerForId(id).toolType;
            px = mCurrentRawPointerData.pointerForId(id).x;
            py = mCurrentRawPointerData.pointerForId(id).y;

            isToolSwitchRangeOut = isRangeOut(mPointerTrack[id].lastX, mPointerTrack[id].lastY,
                                              px, py, mToolSwitchRange);

            isEraserLatchRangeOut = isRangeOut(mPointerTrack[id].startX, mPointerTrack[id].startY,
                                               px, py, mEraserLatchRange);
        }

        if (mPointerTrack[id].isTouching) {
            if (idUpPointers.hasBit(id)) {
#if DEBUG_TOOL_SWITCH
                ALOGD("toolSwitch: id %d, UP", id);
#endif
                mPointerTrack[id].isTouching = false;
                mPointerTrack[id].lastUpTime = when;
            }
            else {
                if (mPointerTrack[id].eraserChance &&
                    mPointerTrack[id].toolType != AMOTION_EVENT_TOOL_TYPE_ERASER) {
                    if (isEraserLatchRangeOut) {
#if DEBUG_TOOL_SWITCH
                        ALOGD("toolSwitch: id %d, no more eraser chance!!!", id);
#endif
                        mPointerTrack[id].eraserChance = false;
                    }
                    else if ((isEraserTimeout || !mPointerTrack[id].firstEraser) &&
                            (tooltype == AMOTION_EVENT_TOOL_TYPE_ERASER)) {
#if DEBUG_TOOL_SWITCH
                        ALOGD("toolSwitch: id %d, firstEraser %d, fake lift to be ERASER",
                              id, mPointerTrack[id].firstEraser);
#endif
                        mPointerTrack[id].toolType = AMOTION_EVENT_TOOL_TYPE_ERASER;
                        mPointerTrack[id].isTouching = false;
                        mPointerTrack[id].lastUpTime = when;
                        ...
                        mCurrentRawPointerData.clearIdBit(id);
                        idCurrentPointers.clearBit(id);
                    }
                }
                mPointerTrack[id].lastX = px;
                mPointerTrack[id].lastY = py;
            }
        }
        else {
            if (idDownPointers.hasBit(id)) {
#if DEBUG_TOOL_SWITCH
                ALOGD("toolSwitch: id %d, DOWN", id);
#endif
                mPointerTrack[id].isTouching = true;
                if (isSwitchTimeout || isToolSwitchRangeOut) {
#if DEBUG_TOOL_SWITCH
                    ALOGD("toolSwitch: new pointer, id %d, type - %s",
                        id, GET_TOOLTYPE_STR(tooltype));
#endif
                    // initialize tracking data from new pointer
                    mPointerTrack[id].startX = mPointerTrack[id].lastX = px;
                    mPointerTrack[id].startY = mPointerTrack[id].lastY = py;
                    mPointerTrack[id].toolType = tooltype;
                    mPointerTrack[id].eraserChance = true;
                    mPointerTrack[id].lastDownTime = when;
                    mPointerTrack[id].firstEraser = tooltype == AMOTION_EVENT_TOOL_TYPE_ERASER;
                    if (mPointerTrack[id].firstEraser) {
#if DEBUG_TOOL_SWITCH
                        ALOGD("toolSwitch: id %d, first eraser, forcing pen", id);
#endif
                        mPointerTrack[id].toolType = AMOTION_EVENT_TOOL_TYPE_STYLUS;
                    }

                    // initialize weighted average filter data here
                    initWeightedAVGFilter(id);
                }
                else if (mPointerTrack[id].eraserChance &&
                         mPointerTrack[id].toolType != AMOTION_EVENT_TOOL_TYPE_ERASER &&
                         tooltype == AMOTION_EVENT_TOOL_TYPE_ERASER &&
                         !isEraserLatchRangeOut) {
#if DEBUG_TOOL_SWITCH
                    ALOGD("toolSwitch: tool changing to eraser");
#endif
                    mPointerTrack[id].toolType = AMOTION_EVENT_TOOL_TYPE_ERASER;
                }
            }
            else if (isSwitchTimeout) {
                // Timed out without next DOWN event, remove the ID from tracking list.
                mIdTrackingPointers.clearBit(id);
            }
        }

        // force tool type
        if (mPointerTrack[id].isTouching && mPointerTrack[id].toolType != tooltype) {
#if DEBUG_TOOL_SWITCH
            ALOGD("!!!! Force tool change: %s -> %s", GET_TOOLTYPE_STR(tooltype),
                GET_TOOLTYPE_STR(mPointerTrack[id].toolType));
#endif
            uint32_t index = mCurrentRawPointerData.idToIndex[id];
            mCurrentRawPointerData.pointers[index].toolType = mPointerTrack[id].toolType;
        }
    }

    mIdLastValidPointers = idCurrentPointers;
}

void TouchInputMapper::NVPreProcTouchInput(nsecs_t when) {

    // Filter out pen and eraser pointers which are started from edge
    // of the screen except top side.
    filterOutEdgePointers();

    // Track the single stylus pointer to allow/lock switching tool type
    trackToolSwitch(when);

    // Only one single pointer is allowed for pen and eraser.
    // Multiple pointers are allowed for only finger.
    // And finger, pen and eraser are mutally exclusive.
    filterOutSubPointers();

    // "Touch Blackout" will block touch input during stylus or in a timeout afterwards
    // to compensate for poor palm rejection by some touch vendors
    updateBlackout(when);
    blockPointersForBlackout(when);
}
...
void TouchInputMapper::sync(nsecs_t when) {
    ...
    if (mDeviceMode == DEVICE_MODE_DISABLED) {
        ...
        // Pre-processing input events to compensate incomplete
        // tool-type classification and palm rejection.
        NVPreProcTouchInput(when);
        ...
        if (mDeviceMode == DEVICE_MODE_POINTER) {
            ...
            if (mDeviceMode == DEVICE_MODE_DIRECT && 
                (mConfig.showTouches || mConfig.showStylus || mConfig.showEraser) &&
                mPointerController != NULL) {
                if (mConfig.showTouches) {
                    mPointerController->setPresentation(PointerControllerInterface::PRESENTATION_SPOT);
                    mPointerController->fade(PointerControllerInterface::TRANSITION_GRADUAL);
                } else if (mConfig.showStylus || mConfig.showEraser) {
                    mPointerController->fade(PointerControllerInterface::TRANSITION_IMMEDIATE);

                    if (!mConfig.showEraser) {
                        // just stylus
                        mPointerController->setPresentation(PointerControllerInterface::PRESENTATION_STYLUS);
                    } else if (!mConfig.showStylus) {
                        // just eraser
                        mPointerController->setPresentation(PointerControllerInterface::PRESENTATION_ERASER);
                    } else  {
                        // both!
                        mPointerController->setPresentation(PointerControllerInterface::PRESENTATION_STYLUS_AND_ERASER);
                    }
                }
...
void TouchInputMapper::dispatchTouches(nsecs_t when, uint32_t policyFlags) {
    ...
    // Apply IIR filter for smoothing pressure
    if (mIIRLowPassFactor < 1.0f || mPressureEBC > 0.0f) {
        // If we are in finger only mode or we are not doing tool type lock.
        // init WeightedAVG filter data for only DOWN IDs
        if (mFingerOnlyMode || !(mToolSwitchTime || mToolSwitchRange ||
            mEraserLatchRange || mEraserDelayTime)) {
            BitSet32 downIdBits(currentIdBits.value & ~lastIdBits.value);
            while (!downIdBits.isEmpty()) {
                uint32_t downId = downIdBits.clearFirstMarkedBit();
                initWeightedAVGFilter(downId);
            }
        }

        BitSet32 filterIdBits(currentIdBits.value);
        if (mIIRLowPassFactor < 1.0f) {
            // Apply filter to only MOVE IDs in case of IIR filter.
            filterIdBits.value &= lastIdBits.value;
        }
        while (!filterIdBits.isEmpty()) {
            uint32_t filterId = filterIdBits.clearFirstMarkedBit();

            // Get last and current pressure
            PointerCoords &lastCoords = mLastCookedPointerData.pointerCoords[filterId];
            PointerCoords &currentCoords = mCurrentCookedPointerData.pointerCoords[filterId];
            float lastPressure = lastCoords.getAxisValue(AMOTION_EVENT_AXIS_PRESSURE);
            float pressure = currentCoords.getAxisValue(AMOTION_EVENT_AXIS_PRESSURE);
#if DEBUG_PRESSURE_SMOOTHING
            float origPressure = pressure;
#endif
            if (mIIRLowPassFactor < 1.0f)
                pressure = applyIIRLowPassFilter(pressure, lastPressure);
            else if (mPressureEBC > 0.0f)
                pressure = applyWeightedAVGFilter(filterId, pressure);
            currentCoords.setAxisValue(AMOTION_EVENT_AXIS_PRESSURE, pressure);

#if DEBUG_PRESSURE_SMOOTHING
            ALOGD("PRESSURE_FILTER id: %d, idx: %d, pressure: %f, filtered_pressure: %f, last: %f",
                filterId, mCurrentCookedPointerData.idToIndex[filterId], origPressure, pressure, lastPressure);
#endif
...
float TouchInputMapper::applyIIRLowPassFilter(float input, float lastOutput) {
    return (lastOutput * (1.000000f - mIIRLowPassFactor) + input * mIIRLowPassFactor);
}

void TouchInputMapper::initWeightedAVGFilter(uint32_t id) {
#if DEBUG_PRESSURE_SMOOTHING
    ALOGD("PRESSURE_FILTER id: %d, reset data for weighted average filter", id);
#endif
    mPressureIdx[id] = 0;
    mPressureCnt[id] = 0;
}

float TouchInputMapper::applyWeightedAVGFilter(uint32_t id, float input) {
    float pressure = 0.0f;
    float w = 1.0f;
    float weight = 0.0f;
...
#if DEBUG_PRESSURE_SMOOTHING
    ALOGD("PRESSURE_FILTER id: %d, apply input %f, idx %d, count %d",
            id, input, mPressureIdx[id], mPressureCnt[id]);
#endif

    mPressures[id][mPressureIdx[id]] = input;
    if (mPressureCnt[id] < MAX_PRESSURE_BUFFER)
                mPressureCnt[id]++;

    idx = mPressureIdx[id];

    for (int i = 0; i < mPressureCnt[id]; i++) {
        pressure += (mPressures[id][idx] * w);
        weight += w;
        w *= mPressureEBC;
        if (idx == 0)
            idx = MAX_PRESSURE_BUFFER - 1;
        else
            idx--;
    }
    pressure /= weight;

    if (mPressureIdx[id] == MAX_PRESSURE_BUFFER - 1)
                mPressureIdx[id] = 0;
    else
                mPressureIdx[id]++;
...
void TouchInputMapper::dispatchPointerGestures(nsecs_t when, uint32_t policyFlags,
    ...
    if (mParameters.gestureMode == Parameters::GESTURE_MODE_SPOTS) {
        ...
        mPointerController->setSpots(mPointerGesture.currentGestureCoords,
                                     NULL, // TODO something better here
...
void JoystickInputMapper::configure(nsecs_t when,
    ...
    if (!changes || (changes & InputReaderConfiguration::CHANGE_MOUSE_FEATURES)) {
        InputFilter::RSMouse::setVirtualMouseState(config->useRSMouse);
    }
}
