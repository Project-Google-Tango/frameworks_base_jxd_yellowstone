...
void PointerController::setSpots(const PointerCoords* spotCoords,
    ...
    for (BitSet32 idBits(spotIdBits); !idBits.isEmpty(); ) {
        ...
        const PointerCoords& c = spotCoords[spotIdToIndex[id]];
        int32_t toolType = props[spotIdToIndex[id]].toolType;

        if (// We're presenting just stylus and tool doesn't match
            ((mLocked.presentation == PRESENTATION_STYLUS) &&
             (toolType != AMOTION_EVENT_TOOL_TYPE_STYLUS)) ||
            // We're presenting just eraser and tool doesn't match
            ((mLocked.presentation == PRESENTATION_ERASER) &&
             (toolType != AMOTION_EVENT_TOOL_TYPE_ERASER)) ||
            // We're presenting both and tool is neither
            ((mLocked.presentation == PRESENTATION_STYLUS_AND_ERASER) &&
             (toolType != AMOTION_EVENT_TOOL_TYPE_ERASER) &&
             (toolType != AMOTION_EVENT_TOOL_TYPE_STYLUS))) {
            continue;
        }
        ...
        const SpriteIcon& icon =
            c.getAxisValue(AMOTION_EVENT_AXIS_PRESSURE) > 0 ?
            ((mLocked.presentation == PRESENTATION_SPOT || props == NULL)?
             mResources.spotTouch :
             (toolType == AMOTION_EVENT_TOOL_TYPE_ERASER
              ? mResources.spotEraser : mResources.spotStylus))
    ...
    for (size_t i = 0; i < mLocked.spots.size(); i++) {
        ...
        if (spot->id != Spot::INVALID_ID) {
            if (!spotIdBits.hasBit(spot->id)) {
                    fadeOutAndReleaseSpotLocked(spot);
            } else {
                int32_t toolType = props[spotIdToIndex[spot->id]].toolType;
                if ((mLocked.presentation == PRESENTATION_STYLUS_AND_ERASER &&
                     toolType != AMOTION_EVENT_TOOL_TYPE_ERASER &&
                     toolType != AMOTION_EVENT_TOOL_TYPE_STYLUS) ||
                    (mLocked.presentation == PRESENTATION_STYLUS &&
                     toolType != AMOTION_EVENT_TOOL_TYPE_STYLUS) ||
...
void PointerController::doAnimate() {
    ...
    for (size_t i = 0; i < mLocked.spots.size(); i++) {
        Spot* spot = mLocked.spots.itemAt(i);
        if (spot->id == Spot::INVALID_ID) {
            if (mLocked.presentation == PRESENTATION_SPOT) {
...
void PointerController::updatePointerLocked() {
    ...
    if (mLocked.pointerIconChanged || mLocked.presentationChanged) {
        // TODO do we need to set something different here for cross/eraser from pointer?
