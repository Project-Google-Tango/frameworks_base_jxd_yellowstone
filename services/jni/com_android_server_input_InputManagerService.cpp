...
public:
    ...
    void setUseRSMouse(bool enabled);
    void SetCursorVisibility(bool visible);
    void setShowTouches(bool enabled);
    void setShowStylus(bool enabled);
    void setShowEraser(bool enabled);
    void setPenToTouchDelaysMs(int delayMs);
    void setTouchLock(bool locked);
    void setFingerOnlyMode(bool enabled);
    void setStylusWhitelistedApp(bool enabled);
    void setStylusLeftHandedMode(bool enabled);
...
private:
    ...
    struct Locked {
        ...
        // Use controller right stick as mouse
        bool useRSMouse;

        //  Mouse Mode
        //   0 = Normal mouse
        //   1 = Virtual Touch mouse
        int mouseMode;
        ...
        // Show stylus feature enable/disable.
        bool showStylus;

        // Show eraser feature enable/disable.
        bool showEraser;

        // -1 for not set
        // 0 for disabled
        // >0 time in ms
        int penToTouchDelayMs;

        // True to lockout touch on system level
        bool lockTouch;

        // True to set finger only mode
        bool fingerOnlyMode;

        // True to notify current foreground app is in the stylus whitelist
        bool stylusWhitelistedApp;

        // false for right-handed mode
...
NativeInputManager::NativeInputManager(jobject contextObj,
...

    {
        ...
        mLocked.mouseMode = 0;
        mLocked.useRSMouse = false;
        mLocked.pointerGesturesEnabled = true;
        mLocked.showTouches = false;
        mLocked.showStylus = false;
        mLocked.showEraser = false;
        mLocked.penToTouchDelayMs = -1;
        mLocked.lockTouch = false;
        mLocked.fingerOnlyMode = true;
        mLocked.stylusWhitelistedApp = false;
        mLocked.leftHandedMode = false;
    }
...
void NativeInputManager::getReaderConfiguration(InputReaderConfiguration* outConfig) {
    ...
    { // acquire lock
        ...
        outConfig->showStylus = mLocked.showStylus;
        outConfig->showEraser = mLocked.showEraser;
        outConfig->penToTouchDelayMs = mLocked.penToTouchDelayMs;
        outConfig->lockTouch = mLocked.lockTouch;
        outConfig->fingerOnlyMode = mLocked.fingerOnlyMode;
        outConfig->stylusWhitelistedApp = mLocked.stylusWhitelistedApp;
        outConfig->leftHandedMode = mLocked.leftHandedMode;

        outConfig->mouseMode = mLocked.mouseMode;
        outConfig->useRSMouse = mLocked.useRSMouse;
...
void NativeInputManager::setUseRSMouse(bool enabled) {
    { // acquire lock
        AutoMutex _l(mLock);

        if (mLocked.useRSMouse == enabled) {
            return;
        }

        ALOGI("Setting right stick mouse feature to %s.", enabled ? "enabled" : "disabled");
        mLocked.useRSMouse = enabled;
    } // release lock

    mInputManager->getReader()->requestRefreshConfiguration(
            InputReaderConfiguration::CHANGE_MOUSE_FEATURES);
}

void NativeInputManager::setMouseMode(int mode) {
    { // acquire lock
        AutoMutex _l(mLock);

        if (mLocked.mouseMode == mode) {
            return;
        }

        mLocked.mouseMode = mode;
    } // release lock
...
void NativeInputManager::setShowStylus(bool enabled) {
    { // acquire lock
        AutoMutex _l(mLock);

        if (mLocked.showStylus == enabled) {
            return;
        }

        ALOGI("Setting show stylus feature to %s.", enabled ? "enabled" : "disabled");
        mLocked.showStylus = enabled;
    } // release lock
...
void NativeInputManager::setShowEraser(bool enabled) {
    { // acquire lock
        AutoMutex _l(mLock);

        if (mLocked.showEraser == enabled) {
            return;
        }

        ALOGI("Setting show eraser feature to %s.", enabled ? "enabled" : "disabled");
        mLocked.showEraser = enabled;
    } // release lock
...
void NativeInputManager::setPenToTouchDelaysMs(int delayMs) {
    { // acquire lock
        AutoMutex _l(mLock);

        if (mLocked.penToTouchDelayMs == delayMs) {
            return;
        }

        ALOGI("Setting penToTouchDelayMs to %d", delayMs);
        mLocked.penToTouchDelayMs = delayMs;
    } // release lock

    mInputManager->getReader()->requestRefreshConfiguration(
            InputReaderConfiguration::CHANGE_BLACKOUT);
}

void NativeInputManager::setTouchLock(bool locked) {
    { // acquire lock
        AutoMutex _l(mLock);

        if (mLocked.lockTouch == locked) {
            return;
        }

        ALOGI("Setting touch lockout state to %s.", locked ? "locked" : "unlocked");
        mLocked.lockTouch = locked;
    } // release lock
...
void NativeInputManager::setFingerOnlyMode(bool enabled) {
    { // acquire lock
        AutoMutex _l(mLock);

        if (mLocked.fingerOnlyMode == enabled) {
            return;
        }

        mLocked.fingerOnlyMode = enabled;
        ALOGI("Setting finger only mode to %d.", mLocked.fingerOnlyMode);
    } // release lock

    mInputManager->getReader()->requestRefreshConfiguration(
            InputReaderConfiguration::CHANGE_FINGER_ONLY_MODE);
}

void NativeInputManager::setStylusWhitelistedApp(bool enabled) {
    { // acquire lock
        AutoMutex _l(mLock);

        if (mLocked.stylusWhitelistedApp == enabled) {
            return;
        }

        mLocked.stylusWhitelistedApp = enabled;
        ALOGI("Setting stylus whitelisted app info to %d.", mLocked.stylusWhitelistedApp);
    } // release lock
...
void NativeInputManager::setStylusLeftHandedMode(bool enabled) {
    { // acquire lock
        AutoMutex _l(mLock);

        if (mLocked.leftHandedMode == enabled) {
            return;
        }

        ALOGI("Setting stylus left-handed mode feature to %s.", enabled ? "enabled" : "disabled");
        mLocked.leftHandedMode = enabled;
    } // release lock

    mInputManager->getReader()->requestRefreshConfiguration(
            InputReaderConfiguration::CHANGE_LEFT_HANDED_MODE);
}
...
void NativeInputManager::loadPointerResources(PointerResources* outResources) {
    ...
    loadSystemIconAsSprite(env, mContextObj, POINTER_ICON_STYLE_SPOT_ERASER,
            &outResources->spotEraser);
    loadSystemIconAsSprite(env, mContextObj, POINTER_ICON_STYLE_SPOT_STYLUS,
            &outResources->spotStylus);
}
...
static void nativeSetUseRSMouse(JNIEnv* env,
        jclass clazz, jint ptr, jboolean enabled) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setUseRSMouse(enabled);
}

static void nativeSetMouseMode(JNIEnv* env,
        jclass clazz, jint ptr, jint mode) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setMouseMode(mode);
}
...
static void nativeSetShowStylus(JNIEnv* env, jclass clazz, jint ptr, jboolean enabled) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setShowStylus(enabled);
}

static void nativeSetShowEraser(JNIEnv* env, jclass clazz, jint ptr, jboolean enabled) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setShowEraser(enabled);
}

static void nativeSetPenToTouchDelayMs(JNIEnv* env, jclass clazz, jint ptr, jint delayMs) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setPenToTouchDelaysMs(delayMs);
}

static void nativeSetTouchLock(JNIEnv* env, jclass clazz, jint ptr, jboolean locked) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setTouchLock(locked);
}

static void nativeSetFingerOnlyMode(JNIEnv* env, jclass clazz, jint ptr, jboolean enabled) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setFingerOnlyMode(enabled);
}

static void nativeSetStylusWhitelistedApp(JNIEnv* env, jclass clazz, jint ptr, jboolean enabled) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setStylusWhitelistedApp(enabled);
}

static void nativeSetStylusLeftHandedMode(JNIEnv* env, jclass clazz, jint ptr, jboolean enabled) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setStylusLeftHandedMode(enabled);
}
...
static JNINativeMethod gInputManagerMethods[] = {
    ...
    { "nativeSetMouseMode", "(II)V",
            (void*) nativeSetMouseMode },
    { "nativeSetUseRSMouse", "(IZ)V",
            (void*) nativeSetUseRSMouse },
    {"nativeSetCursorVisibility", "(IZ)V",
            (void*) nativeSetCursorVisibility},
    { "nativeSetShowTouches", "(IZ)V",
            (void*) nativeSetShowTouches },
    { "nativeSetShowStylus", "(IZ)V",
            (void*) nativeSetShowStylus },
    { "nativeSetShowEraser", "(IZ)V",
            (void*) nativeSetShowEraser },
    { "nativeSetPenToTouchDelayMs", "(II)V",
            (void*) nativeSetPenToTouchDelayMs },
    { "nativeSetTouchLock", "(IZ)V",
            (void*) nativeSetTouchLock },
    { "nativeSetFingerOnlyMode", "(IZ)V",
            (void*) nativeSetFingerOnlyMode},
    { "nativeSetStylusWhitelistedApp", "(IZ)V",
            (void*) nativeSetStylusWhitelistedApp},
    { "nativeSetStylusLeftHandedMode", "(IZ)V",
            (void*) nativeSetStylusLeftHandedMode },
