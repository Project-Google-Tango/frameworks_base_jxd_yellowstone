...
class AppWindowToken extends WindowToken {
    ...
    /*
     * notify all the windows in an app that they're activated/inactivated
     * when this app changes visible/invisible
     */
    void activateWindows(boolean active) {
        try {
            final int N = allAppWindows.size();
            for (int i=0; i<N; i++) {
                WindowState win = allAppWindows.get(i);
                try {
                    win.mClient.activateWindow(active);
    ...
    /*
     * notify window that it'll be inactivated
     * before it's removed from windows list
     */
    void preRemoveWindow(WindowState win) {
        try {
            win.mClient.activateWindow(false);
        } catch (RemoteException e) {
        }
    }
