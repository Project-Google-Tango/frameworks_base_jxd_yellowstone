...
import static android.net.wifi.WifiManager.WIFI_MODE_STA_LOW_LATENCY;
import static android.net.wifi.WifiManager.WIFI_MODE_DISABLE_AMPDU;
...
class WifiController extends StateMachine {
    ...
    /**
     * Command for new feature. Three categories: work_mode, whitelist, blacklist.
     */
    static final int CMD_NV_FLT_WORK_MODE              = 0;
    static final int CMD_NV_FLT_WHITE_TCP_PORT_ADD     = 1;
    static final int CMD_NV_FLT_WHITE_TCP_PORT_REMOVE  = 2;
    static final int CMD_NV_FLT_WHITE_UDP_PORT_ADD     = 3;
    static final int CMD_NV_FLT_WHITE_UDP_PORT_REMOVE  = 4;
    static final int CMD_NV_FLT_WHITE_PORTS_CLEAR      = 5;
    static final int CMD_NV_FLT_BLACK_TCP_PORT_ADD     = 6;
    static final int CMD_NV_FLT_BLACK_TCP_PORT_REMOVE  = 7;
    static final int CMD_NV_FLT_BLACK_UDP_PORT_ADD     = 8;
    static final int CMD_NV_FLT_BLACK_UDP_PORT_REMOVE  = 9;
    static final int CMD_NV_FLT_BLACK_PORTS_CLEAR      = 10;
    static final int CMD_NV_FLT_SUPPORTED              = 11;

    /**
     * Work mode: "0"-disable; "1"-whitelist; "2"-blacklist.
     */
    static final String CMD_NV_FLT_WORK_MODE_DISABLE     = "0";
    static final String CMD_NV_FLT_WORK_MODE_WHITELIST   = "1";
    static final String CMD_NV_FLT_WORK_MODE_BLACKLIST   = "2";

    //action to receive broadcast for the new feature.
    static final String ACTION_NV_FLT_WORK_MODE =
                                "com.nvidia.action.NV_FLT.WORK_MODE";
    static final String ACTION_NV_FLT_PORT =
                                "com.nvidia.action.NV_FLT.PORT";
    static final String ACTION_NV_FLT_SUPPORTED =
                                "com.nvidia.action.NV_FLT.FLT_SUPPORTED";
    ...
    private LowLatencyLockHeldState mLowLatencyLockHeldState = new LowLatencyLockHeldState();
    /* Below locks are for active states */
    private FullLockHeldActiveState mFullLockHeldActiveState = new FullLockHeldActiveState();
    private FullHighPerfLockHeldActiveState mFullHighPerfLockHeldActiveState = new FullHighPerfLockHeldActiveState();
    private LowLatencyLockHeldActiveState mLowLatencyLockHeldActiveState = new LowLatencyLockHeldActiveState();
    private NoLockHeldActiveState mNoLockHeldActiveState = new NoLockHeldActiveState();
    private DisableAmpduLockHeldActiveState mDisableAmpduLockHeldActiveState = new DisableAmpduLockHeldActiveState();
        ...
        addState(mDefaultState);
            ...
            addState(mStaEnabledState, mDefaultState);
                addState(mDeviceActiveState, mStaEnabledState);
                    /* Below states are for active states */
                    addState(mFullLockHeldActiveState, mDeviceActiveState);
                    addState(mFullHighPerfLockHeldActiveState, mDeviceActiveState);
                    addState(mLowLatencyLockHeldActiveState, mDeviceActiveState);
                    addState(mNoLockHeldActiveState, mDeviceActiveState);
                    addState(mDisableAmpduLockHeldActiveState, mDeviceActiveState);
                addState(mDeviceInactiveState, mStaEnabledState);
                    addState(mScanOnlyLockHeldState, mDeviceInactiveState);
                    addState(mFullLockHeldState, mDeviceInactiveState);
                    addState(mFullHighPerfLockHeldState, mDeviceInactiveState);
                    addState(mLowLatencyLockHeldState, mDeviceInactiveState);
    ...
    class DeviceActiveState extends State {
        ...
        public void enter() {
            ...
            mWifiStateMachine.setStaLowLatency(false);
        }
        ...
        public boolean processMessage(Message msg) {
            switch(msg.what) {
                case CMD_DEVICE_IDLE:
                     mDeviceIdle = true;
                     logd("device idle, transitioning to mDeviceInactiveState");
                     checkLocksAndTransitionWhenDeviceIdle();
                     break;
                case CMD_LOCKS_CHANGED:
                     logd("calling checkLocksDeviceActive");
                     checkLocksDeviceActive();
                ...
                case CMD_USER_PRESENT:
                    ...
                    break;
                default:
                    loge("Not handled here " + msg.what);
                    return NOT_HANDLED;
            }
            return HANDLED;
        }
    ...
    /* Parent: DeviceActiveState , App holding full lock */
    class FullLockHeldActiveState extends State {
        @Override
        public void enter() {
            mWifiStateMachine.setHighPerfModeEnabled(false);
            mWifiStateMachine.setStaLowLatency(false);
    ...
    /* Parent: DeviceActiveState , App holding full high perf lock */
    class FullHighPerfLockHeldActiveState extends State {
        @Override
        public void enter() {
    ...
    /* Parent: DeviceActiveState , App holding low latency lock */
    class LowLatencyLockHeldActiveState extends State {
        @Override
        public void enter() {
            mWifiStateMachine.setHighPerfModeEnabled(false);
            mWifiStateMachine.setStaLowLatency(true);
        }
    ...
    /* Parent: DeviceActiveState , App holding no lock */
    class NoLockHeldActiveState extends State {
        @Override
        public void enter() {
            logd("Entered no lock held mode");
            mWifiStateMachine.setHighPerfModeEnabled(false);
            mWifiStateMachine.setStaLowLatency(false);
        }
    }

    /* Parent: DeviceActiveState, App holding disable AMPDU lock */
    class DisableAmpduLockHeldActiveState extends State {
        @Override
        public void enter() {
            mWifiStateMachine.setDisableAmpdu(true);
        }
        @Override
        public void exit() {
            mWifiStateMachine.setDisableAmpdu(false);
        }
    }
    ...
    class LowLatencyLockHeldState extends State {
        public void enter() {
            mWifiStateMachine.setStaLowLatency(true);
        }
        @Override
    ...
    private void checkLocksDeviceActive() {
        logd("entered checkLocksDeviceActive " + getCurrentState().getName());
        if (mLocks.hasLocks()) {
            switch (mLocks.getStrongestLockMode()) {
                case WIFI_MODE_FULL:
                    if (!mFullLockHeldActiveState.equals(getCurrentState()))
                        transitionTo(mFullLockHeldActiveState);
                    break;
                case WIFI_MODE_FULL_HIGH_PERF:
                    if (!mFullHighPerfLockHeldActiveState.equals(getCurrentState()))
                        transitionTo(mFullHighPerfLockHeldActiveState);
                    break;
                case WIFI_MODE_STA_LOW_LATENCY:
                    if (!mLowLatencyLockHeldActiveState.equals(getCurrentState()))
                        transitionTo(mLowLatencyLockHeldActiveState);
                    break;
                case WIFI_MODE_DISABLE_AMPDU:
                    transitionTo(mDisableAmpduLockHeldActiveState);
                    break;
                default:
                    loge("Illegal lock " + mLocks.getStrongestLockMode());
                    transitionTo(mNoLockHeldActiveState);
            }
        } else {
            logd("Entering no lock held mode");
            transitionTo(mNoLockHeldActiveState);
        }
    }
        ...
        if (mLocks.hasLocks()) {
            switch (mLocks.getStrongestLockMode()) {
                ...
                case WIFI_MODE_SCAN_ONLY:
                    ...
                    break;
                case WIFI_MODE_STA_LOW_LATENCY:
                    transitionTo(mLowLatencyLockHeldState);
                    break;
                default:
                    loge("Illegal lock " + mLocks.getStrongestLockMode());
            }
