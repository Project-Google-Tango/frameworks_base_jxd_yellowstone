...
import static com.android.server.wifi.WifiController.CMD_NV_FLT_WORK_MODE;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_WHITE_TCP_PORT_ADD;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_WHITE_TCP_PORT_REMOVE;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_WHITE_UDP_PORT_ADD;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_WHITE_UDP_PORT_REMOVE;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_WHITE_PORTS_CLEAR;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_BLACK_TCP_PORT_ADD;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_BLACK_TCP_PORT_REMOVE;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_BLACK_UDP_PORT_ADD;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_BLACK_UDP_PORT_REMOVE;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_BLACK_PORTS_CLEAR;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_WORK_MODE_DISABLE;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_WORK_MODE_WHITELIST;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_WORK_MODE_BLACKLIST;
import static com.android.server.wifi.WifiController.CMD_NV_FLT_SUPPORTED;
import static com.android.server.wifi.WifiController.ACTION_NV_FLT_WORK_MODE;
import static com.android.server.wifi.WifiController.ACTION_NV_FLT_PORT;
import static com.android.server.wifi.WifiController.ACTION_NV_FLT_SUPPORTED;
...
public final class WifiService extends IWifiManager.Stub {
    ...
    private int mLowLatencyLocksAcquired;
    private int mLowLatencyLocksReleased;
    private int mDisableAmpduLocksAcquired;
    private int mDisableAmpduLocksReleased;
    ...
    public WifiService(Context context) {
        ...
        registerBroadcastsForNvFilters();
    }
    ...
    private BroadcastReceiver mNvFiltersReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(ACTION_NV_FLT_WORK_MODE)) {
                if ((workMode = intent.getStringExtra("work_mode")) != null) {
                    if (workMode.equals(CMD_NV_FLT_WORK_MODE_DISABLE)) {
                        Slog.i(TAG, "NV_FLT work mode is: disable!");
                        mWifiStateMachine.configNvFilters(mWifiStateMachineChannel, CMD_NV_FLT_WORK_MODE,
                               CMD_NV_FLT_WORK_MODE_DISABLE);
                    } else if (workMode.equals(CMD_NV_FLT_WORK_MODE_WHITELIST)) {
                        Slog.i(TAG, "NV_FLT work mode is: whitelist!");
                        mWifiStateMachine.configNvFilters(mWifiStateMachineChannel, CMD_NV_FLT_WORK_MODE,
                               CMD_NV_FLT_WORK_MODE_WHITELIST);
                    } else if (workMode.equals(CMD_NV_FLT_WORK_MODE_BLACKLIST)) {
                        Slog.i(TAG, "NV_FLT work mode is: Blacklist!");
                        mWifiStateMachine.configNvFilters(mWifiStateMachineChannel, CMD_NV_FLT_WORK_MODE,
                               CMD_NV_FLT_WORK_MODE_BLACKLIST);
                    } else {
                        Slog.e(TAG, "Unexpected NV_FLT work mode.");
                    }
                }
            } else if (action.equals(ACTION_NV_FLT_PORT)) {
                if ((portList = intent.getIntegerArrayListExtra("whitelist_port_data")) != null) {
                    StringBuilder sb = new StringBuilder();
                    Iterator<Integer> iter = portList.iterator();
                    while (iter.hasNext()) {
                        String port = Integer.toString(iter.next());
                        sb.append(port);
                        sb.append(";");
                    }
                    Slog.i(TAG, "Add port " + sb.toString() + " for Whitelist.");
                    mWifiStateMachine.configNvFilters(mWifiStateMachineChannel,
                            CMD_NV_FLT_WHITE_PORTS_CLEAR, "0");
                    mWifiStateMachine.configNvFilters(mWifiStateMachineChannel,
                            CMD_NV_FLT_WHITE_TCP_PORT_ADD, sb.toString());
                } else if ((portList = intent.getIntegerArrayListExtra("blacklist_port_data")) != null) {
                    StringBuilder sb = new StringBuilder();
                    Iterator<Integer> iter = portList.iterator();
                    while (iter.hasNext()) {
                        String port = Integer.toString(iter.next());
                    ...
                    Slog.i(TAG, "Add port " + sb.toString() + " for Blacklist.");
                    mWifiStateMachine.configNvFilters(mWifiStateMachineChannel,
                            CMD_NV_FLT_BLACK_PORTS_CLEAR, "0");
                    mWifiStateMachine.configNvFilters(mWifiStateMachineChannel,
                            CMD_NV_FLT_BLACK_TCP_PORT_ADD, sb.toString());
                } else {
                    Slog.e(TAG, "Unexpected intent data for NV_FLT.");
                }
            } else if (action.equals(ACTION_NV_FLT_SUPPORTED)) {
                File wakeupMonitor = new File("/proc/net/twm_offender_stat");
                String rxFilterSupported = SystemProperties.get("wifi.rx-filter", "0");
                boolean filterSupported = wakeupMonitor.exists()
                        && (mWifiStateMachine.configNvFilters(mWifiStateMachineChannel,
                                CMD_NV_FLT_SUPPORTED, "0")
                        || rxFilterSupported.equals("1"));
                getResultExtras(true).putBoolean("wifi_filter_supported", filterSupported);
                Slog.d(TAG, "NV filter supported: "
                        + getResultExtras(true).getBoolean("wifi_filter_supported"));
            } else {
                Slog.e(TAG, "Unexpected action for NV_FLT.");
            }
        }
    };

    /**
     * Get the maximum link speed.
     */
    public int getMaxLinkSpeed() {
        return mWifiStateMachine.getMaxLinkSpeed();
    }
    ...
    private void registerBroadcastsForNvFilters() {
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction(ACTION_NV_FLT_WORK_MODE);
        intentFilter.addAction(ACTION_NV_FLT_PORT);
        intentFilter.addAction(ACTION_NV_FLT_SUPPORTED);
        mContext.registerReceiver(mNvFiltersReceiver, intentFilter);
    }
    ...
    class LockList {
        ...
        synchronized int getStrongestLockMode() {
            if (mList.isEmpty()) {
                Slog.d(TAG, "returning full");
                return WifiManager.WIFI_MODE_FULL;
            }

            if (mDisableAmpduLocksAcquired > mDisableAmpduLocksReleased) {
                Slog.d(TAG, "returning disable ampdu");
                return WifiManager.WIFI_MODE_DISABLE_AMPDU;
            }

            if (mLowLatencyLocksAcquired > mLowLatencyLocksReleased) {
                Slog.d(TAG, "returning low latency");
                return WifiManager.WIFI_MODE_STA_LOW_LATENCY;
            }

            if (mFullHighPerfLocksAcquired > mFullHighPerfLocksReleased) {
                Slog.d(TAG, "returning full high perf");
            ...
            Slog.d(TAG, "returning scan only");
    ...
    public boolean acquireWifiLock(IBinder binder, int lockMode, String tag, WorkSource ws) {
        ...
        if (lockMode != WifiManager.WIFI_MODE_FULL &&
                ...
                lockMode != WifiManager.WIFI_MODE_STA_LOW_LATENCY &&
                lockMode != WifiManager.WIFI_MODE_DISABLE_AMPDU) {
    ...
    private void noteAcquireWifiLock(WifiLock wifiLock) throws RemoteException {
        switch(wifiLock.mMode) {
            ...
            case WifiManager.WIFI_MODE_STA_LOW_LATENCY:
            case WifiManager.WIFI_MODE_DISABLE_AMPDU:
    ...
    private boolean acquireWifiLockLocked(WifiLock wifiLock) {
        ...
        try {
            ...
            case WifiManager.WIFI_MODE_STA_LOW_LATENCY:
                ++mLowLatencyLocksAcquired;
                break;

            case WifiManager.WIFI_MODE_DISABLE_AMPDU:
                ++mDisableAmpduLocksAcquired;
                break;
            }
            ...
            if (hadLock) {
                ...
                switch(wifiLock.mMode) {
                    ...
                    case WifiManager.WIFI_MODE_SCAN_ONLY:
                        ...
                        break;
                    case WifiManager.WIFI_MODE_STA_LOW_LATENCY:
                        ++mLowLatencyLocksReleased;
                        break;
                    case WifiManager.WIFI_MODE_DISABLE_AMPDU:
                        ++mDisableAmpduLocksReleased;
                        break;
                }
                mWifiController.sendMessage(CMD_LOCKS_CHANGED);
            }
