...
public final class BatteryService extends Binder {
    ...
    private int mShutdownColdBatteryTemperature;
    private boolean mAllowShutdownWithCharger;
    ...
    public BatteryService(Context context, LightsService lights) {
        ...
        mShutdownColdBatteryTemperature = mContext.getResources().getInteger(
                com.android.internal.R.integer.config_shutdownColdBatteryTemperature);
        mAllowShutdownWithCharger = mContext.getResources().getBoolean(
                com.android.internal.R.bool.config_allowShutdownWithCharger);
    ...
    private void shutdownIfNoPowerLocked() {
        // shut down gracefully if our battery is critically low.
        // wait until the system has booted before attempting to display the shutdown dialog.
        boolean shutdownNow = false;
        if (mBatteryProps.batteryLevel == 0) {
            if (mAllowShutdownWithCharger) {
                if (mBatteryProps.batteryStatus != BatteryManager.BATTERY_STATUS_UNKNOWN) {
                    Slog.d(TAG, "shutdown regardless of any charger");
                    shutdownNow = true;
                }
            } else {
    ...
    private void shutdownIfUnderTempLocked() {
        // shut down gracefully if temperature is too low (< -20.0C by default)
        // wait until the system has booted before attempting to display the
        // shutdown dialog.
        if (mBatteryProps.batteryTemperature < mShutdownColdBatteryTemperature) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    if (ActivityManagerNative.isSystemReady()) {
