...
class MountService extends IMountService.Stub
    ...
    class VoldResponseCode {
        ...
        // 690 series - Async obb move (unsolicited)
        public static final int ObbMoveAsyncResult             = 690;
        public static final int ObbMoveAsyncRecoveryResult     = 691;
        ...
        /*
         * 9000 series - Async obb move (non-unsolicited)
         */
        public static final int ObbMoveCheckObb                = 9000;
    }
    ...
    private CountDownLatch mObbMoveAsyncDone = null;
    private boolean mObbMoveAsyncFlag;

    private CountDownLatch mObbMoveAsyncRecoveryDone = null;
    private boolean mObbMoveAsyncRecoveryFlag;
    ...
    private static final int OBB_MOVE_THROUGHPUT = 2; // slow class 2 sdcard: 2MB/s
    ...
    private void updatePublicVolumeState(StorageVolume volume, String state) {
        ...
        // Tell PackageManager about changes to volume state, but only
        // when not emulated.
        if (!volume.isEmulated()) {
    ...
    public boolean onEvent(int code, String raw, String[] cooked) {
        ...
        } else if (code == VoldResponseCode.ObbMoveAsyncResult) {
            Slog.d(TAG, "received ObbMoveAsyncResult code");

            if (mObbMoveAsyncDone != null) {
                final String result = cooked[2];
                mObbMoveAsyncFlag = false;
                if (result.equals("completed")) {
                    mObbMoveAsyncFlag = true;
                }
                mObbMoveAsyncDone.countDown();
                Slog.d(TAG, "countDown mObbMoveAsyncDone and result is " + mObbMoveAsyncFlag);
            }
        } else if (code == VoldResponseCode.ObbMoveAsyncRecoveryResult) {
            Slog.d(TAG, "received ObbMoveAsyncRecoveryResult code");

            if (mObbMoveAsyncRecoveryDone != null) {
                final String result = cooked[2];
                mObbMoveAsyncRecoveryFlag = false;
                if (result.equals("completed")) {
                    mObbMoveAsyncRecoveryFlag = true;
                }
                mObbMoveAsyncRecoveryDone.countDown();
                Slog.d(TAG, "countDown mObbMoveAsyncRecoveryDone and result is " + mObbMoveAsyncRecoveryFlag);
            }
        } else {
            return false;
        }
    ...
    public String[] getCurrentSecureContainerList() {
        validatePermission(android.Manifest.permission.ASEC_ACCESS);
        waitForReady();
        warnOnNotMounted();
    ...
    //
    // Move obb files to sd card
    //

    // [TODO] return obb info object (# of files, total size, ...)
    public long checkObb(String pkgObbDir, boolean checkFile) {
        validatePermission(android.Manifest.permission.MOUNT_UNMOUNT_FILESYSTEMS);
        warnOnNotMounted();

        Slog.e(TAG, "checkObb: pkgObbDir=" + pkgObbDir + ", checkFile=" + checkFile);
        long rc = StorageResultCode.OperationSucceeded;    // 0
        try {
            if (checkFile) {
                final NativeDaemonEvent event;
                event = mConnector.execute("obbmove", "check", pkgObbDir, "check_file");
                rc = StorageResultCode.OperationFailedInternalError;    // -1
                event.checkCode(VoldResponseCode.ObbMoveCheckObb);
                String raw = event.getMessage();
                Slog.e(TAG, "checkObb: received message, raw = " + raw);
                StringTokenizer st = new StringTokenizer(raw);
                if (st.hasMoreElements()) {
                    String result = (String) st.nextElement();
                    if (!result.equals("None")) {
                        rc = Long.parseLong(result);    // total size > 0
                        Slog.e(TAG, "checkObb: total size > 0: rc = " + rc);
                    }
                }
            } else {
                mConnector.execute("obbmove", "check", pkgObbDir);
            }
        } catch (NativeDaemonConnectorException e) {
            rc = StorageResultCode.OperationFailedInternalError;    // -1
            Slog.e(TAG, "checkObb: NativeDaemonConnectorException: " + e);
        }
        return rc;
    }

    public int moveObb(String srcDir, String destDir, long sizeBytes) {
        ...
        mObbMoveAsyncFlag = false;
        mObbMoveAsyncDone = new CountDownLatch(1);

        long asyncTimeOut = sizeBytes / (OBB_MOVE_THROUGHPUT * 1024 * 1024);
        asyncTimeOut += asyncTimeOut / 10 * 2; // 120%
        if (asyncTimeOut < 10) {
            asyncTimeOut = 10;
        }
        ...
        int rc = StorageResultCode.OperationSucceeded;
        try {
            mConnector.execute("obbmove", "move", srcDir, destDir, "async");
        } catch (NativeDaemonConnectorException e) {
            rc = StorageResultCode.OperationFailedInternalError;
        }
        ...
        try {
            if (rc == StorageResultCode.OperationSucceeded) {
                mObbMoveAsyncDone.await(asyncTimeOut, TimeUnit.SECONDS);
                if (mObbMoveAsyncFlag == false) {
                    rc = StorageResultCode.OperationFailedInternalError;
                }
            }
        } catch (InterruptedException e) {
        ...
        mObbMoveAsyncDone = null;
        return rc;
    }

    public int mountObbMove(String srcDir, String destDir) {
        validatePermission(android.Manifest.permission.MOUNT_UNMOUNT_FILESYSTEMS);
        warnOnNotMounted();

        int rc = StorageResultCode.OperationSucceeded;
        try {
            mConnector.execute("obbmove", "bind_mount", srcDir, destDir);
        } catch (NativeDaemonConnectorException e) {
            rc = StorageResultCode.OperationFailedInternalError;
        }
        return rc;
    ...
    public int unmountObbMove(String mountPoint, boolean force) {
        validatePermission(android.Manifest.permission.MOUNT_UNMOUNT_FILESYSTEMS);
        warnOnNotMounted();

        int rc = StorageResultCode.OperationSucceeded;
        try {
            mConnector.execute("obbmove", "bind_unmount", mountPoint, force);
        } catch (NativeDaemonConnectorException e) {
            rc = StorageResultCode.OperationFailedInternalError;
        }
        return rc;
    ...
    public int recoverObb(String srcDir, String destDir, long sizeBytes) {
        //validatePermission(android.Manifest.permission.WRITE_EXTERNAL_STORAGE);
        warnOnNotMounted();
        ...
        mObbMoveAsyncRecoveryFlag = false;
        mObbMoveAsyncRecoveryDone = new CountDownLatch(1);
        ...
        int rc = StorageResultCode.OperationSucceeded;
        try {
            mConnector.execute("obbmove", "recover", srcDir, destDir);
        } catch (NativeDaemonConnectorException e) {
            rc = StorageResultCode.OperationFailedInternalError;
        }
        ...
        try {
            if (rc == StorageResultCode.OperationSucceeded) {
                mObbMoveAsyncRecoveryDone.await(asyncTimeOut, TimeUnit.SECONDS);
                if (mObbMoveAsyncRecoveryFlag == false) {
                    rc = StorageResultCode.OperationFailedInternalError;
                }
            }
        } catch (InterruptedException e) {
        ...
        mObbMoveAsyncRecoveryDone = null;
        return rc;
    }
