...
class AlarmManagerService extends IAlarmManager.Stub {
    ...
    private static final int WAKENUP_BY_RTCCHIP = 11;
    private static final int WAKENUP_BY_RTCCHIP_MASK = 1 << WAKENUP_BY_RTCCHIP;
            ...
            while (true)
            {
                ...
                synchronized (mLock) {
                    ...
                    for (int i=0; i<triggerList.size(); i++) {
                        ...
                        if ((result & WAKENUP_BY_RTCCHIP_MASK) != 0
                                && (alarm.type == AlarmManager.ELAPSED_REALTIME_WAKEUP
                                    || alarm.type == AlarmManager.RTC_WAKEUP)
                                && alarm.when <= min) {
                            targetAlarm = alarm;
                            min = targetAlarm.when;
                            tAlarmValid = true;
                        }
                        try {
                ...
                if ((result & WAKENUP_BY_RTCCHIP_MASK) != 0) {
                    if (tAlarmValid != true) {
                        Slog.w(TAG, "Device is waken up by rtc, but haven't found an offender!");
                        continue;
                    }

                    Intent intent = new Intent("com.nvidia.action.RTC_WAKEUP_OFFENDER");
                    String appName = targetAlarm.operation.getTargetPackage();
                    intent.putExtra("offender.app.name", appName);
                    mContext.sendBroadcastAsUser(intent, UserHandle.ALL);
                    Slog.d(TAG, targetAlarm.toString());
                }
            }
        }
    }
