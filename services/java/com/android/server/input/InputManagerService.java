...
public class InputManagerService extends IInputManager.Stub
    ...
    // --ShieldTech--
    //  RSMouse Support
    // Enables RS mouse if the Settings say so
    public static final int MOUSE_MODE_DEFAULT = 0;
    // Disables RSMouse even if the Settings have it turned on
    public static final int MOUSE_MODE_BLOCK = 1;
    // Not supported currently.  Same behavior as DEFAULT
    public static final int MOUSE_MODE_FORCE = 2;

    private int mAppRSMouseMode;
    ...
    private static final String MAXIM_TOUCH_FUSION_FINGER_ONLY_PATH =
                                "/vendor/bin/touch_fusion finger_only ";
    private static final String STYLUS_REPORT_MODE_PATH =
                                "sys/devices/virtual/misc/touch/report_mode";
    private static final int FINGER_ONLY_MODE_VALUE = 2;
    private boolean mAppStylusFingerOnlyMode;
    private boolean mCurrentStylusFingerOnlyMode;
    private String mStylusEnablePackage;
    private boolean mCurrentTouchLocked;
    ...
    private static native void nativeSetMouseMode(int ptr, int mode);
    private static native void nativeSetUseRSMouse(int ptr, boolean enabled);
    ...
    private static native void nativeSetShowStylus(int ptr, boolean enabled);
    private static native void nativeSetShowEraser(int ptr, boolean enabled);
    private static native void nativeSetPenToTouchDelayMs(int ptr, int delayMs);
    private static native void nativeSetTouchLock(int ptr, boolean locked);
    private static native void nativeSetFingerOnlyMode(int ptr, boolean enabled);
    private static native void nativeSetStylusWhitelistedApp(int ptr, boolean enabled);
    private static native void nativeSetStylusLeftHandedMode(int ptr, boolean enabled);
    ...
    /** Switch code: Stylus pen switch.  When set, pen is removed, i.e. tablet mode. */
    ...
    public InputManagerService(Context context, Handler handler) {
        ...
        mAppRSMouseMode = MOUSE_MODE_DEFAULT;
        mAppStylusFingerOnlyMode = true;
        mCurrentStylusFingerOnlyMode = true;
        mStylusEnablePackage = null;
        mCurrentTouchLocked = false;
    ...
    public void start() {
        ...
        registerMouseModeSettingObserver();
        registerUseRSMouseSettingObserver();
        registerShowTouchesSettingObserver();
        registerShowStylusSettingObserver();
        registerShowEraserSettingObserver();
        registerPenToTouchDelayMsSettingObserver();
        registerStylusFingerOnlyModeSettingObserver();
        registerStylusLeftHandedModeSettingObserver();
        registerUserSetupObserver();
        ...
        mContext.registerReceiver(new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                updateShowStylusFromSettings();
                updateShowEraserFromSettings();
                updatePenToTouchDelayMsFromSettings();
                updateStylusFingerOnlyModeFromSettings();
                updateStylusLeftHandedModeFromSettings();
                updateUserSetupFromSettings();
            }
        }, new IntentFilter(Intent.ACTION_USER_SWITCHED), null, mHandler);

        mContext.registerReceiver(mTouchLockStateReceiver,
            new IntentFilter(WindowManagerPolicy.ACTION_LOCK_TOUCH), null, mHandler);

        mContext.registerReceiverAsUser(mEnableStylusStateReceiver, UserHandle.ALL,
            new IntentFilter(WindowManagerPolicy.ACTION_ENABLE_STYLUS), null, mHandler);

        updatePointerSpeedFromSettings();
        updateMouseModeFromSettings();
        updateUseRSMouseFromSettings();
        updateShowTouchesFromSettings();
        updateShowStylusFromSettings();
        updateShowEraserFromSettings();
        updatePenToTouchDelayMsFromSettings();
    ...
    public void setStylusFingerOnlyMode(boolean fingerOnly) {
        if (mCurrentStylusFingerOnlyMode != fingerOnly) {
            int mode = fingerOnly ? FINGER_ONLY_MODE_VALUE : 0;
            if (new File(STYLUS_REPORT_MODE_PATH).exists()) {
                FileOutputStream fos = null;
                try {
                     fos = new FileOutputStream(STYLUS_REPORT_MODE_PATH);
                     byte[] bytes = new byte[2];
                     bytes[0] = (byte) (0xFF & (mode + '0'));
                     bytes[1] = '\n';
                     fos.write(bytes);
                     mCurrentStylusFingerOnlyMode = fingerOnly;
                     if (DEBUG) Slog.d(TAG, "setStylusFingerOnlyMode: mode is set to "
                         + Integer.toString(mode));
                     nativeSetFingerOnlyMode(mPtr, mCurrentStylusFingerOnlyMode);
                } catch (Exception e) {
                    Log.e(TAG, "Set finger_only mode: " + e.toString());
                } finally {
                ...
                try {
                    java.lang.Process p = Runtime.getRuntime().exec(
                        MAXIM_TOUCH_FUSION_FINGER_ONLY_PATH + Integer.toString(mode));
                    ...
                    if (status == 0) {
                        ...
                        Log.e(TAG, "Failed to set finger_only mode, status = "
                            + Integer.toString(status));
                    }
                } catch (IOException e) {
                    Log.e(TAG, "Set finger_only mode: IOException " + e.toString());
                } catch (InterruptedException e) {
                    Log.e(TAG, "Set finger_only mode: InterruptedException " + e.toString());
                }
            }
        }
    }

    public void setAppStylusFingerOnlyMode(boolean fingerOnly) {
        if (DEBUG) Slog.d(TAG, "setAppStylusFingerOnlyMode: fingerOnly=" + fingerOnly);
        if (mAppStylusFingerOnlyMode != fingerOnly) {
            mAppStylusFingerOnlyMode = fingerOnly;
            updateStylusFingerOnlyModeFromSettings();
            nativeSetStylusWhitelistedApp(mPtr, !mAppStylusFingerOnlyMode);
        }
    }

    public String getStylusEnablePackage() {
        return mStylusEnablePackage;
    }

    public void setAppRSMouseMode(int mode) {
        if (mAppRSMouseMode != mode) {
            mAppRSMouseMode = mode;
            updateUseRSMouseFromSettings();
        }
    }

    public void updateUseRSMouseFromSettings() {
        int setting = Settings.System.getIntForUser(mContext.getContentResolver(), "controller_rsmouse", 1, UserHandle.USER_CURRENT);
        nativeSetUseRSMouse(mPtr, (setting != 0 && mAppRSMouseMode != MOUSE_MODE_BLOCK));
    }

    private void registerUseRSMouseSettingObserver() {
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor("controller_rsmouse"), true,
                new ContentObserver(mHandler) {
                    @Override
                    public void onChange(boolean selfChange) {
                        updateUseRSMouseFromSettings();
    ...
    public void updateMouseModeFromSettings() {
        int setting = Settings.System.getIntForUser(mContext.getContentResolver(), "mouse_mode", 1, UserHandle.USER_CURRENT);
        nativeSetMouseMode(mPtr, setting);
    }

    private void registerMouseModeSettingObserver() {
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor("mouse_mode"), true,
                new ContentObserver(mHandler) {
                    @Override
                    public void onChange(boolean selfChange) {
                        updateMouseModeFromSettings();
    ...
    public void updateShowStylusFromSettings() {
        int setting = getShowStylusSetting(1);
        nativeSetShowStylus(mPtr, setting != 0);
    }

    public void updateShowEraserFromSettings() {
        int setting = getShowEraserSetting(1);
        nativeSetShowEraser(mPtr, setting != 0);
    }

    public void updatePenToTouchDelayMsFromSettings() {
        int setting = getPenToTouchDelayMsSetting(-1);
        nativeSetPenToTouchDelayMs(mPtr, setting);
    }

    public void updateTouchLockState(boolean lockTouch) {
        if (mCurrentTouchLocked != lockTouch) {
            nativeSetTouchLock(mPtr, lockTouch);

            Intent intent = new Intent(WindowManagerPolicy.ACTION_TOUCH_LOCKED);
            intent.putExtra(WindowManagerPolicy.EXTRA_TOUCH_LOCK, lockTouch);
            mContext.sendBroadcastAsUser(intent, UserHandle.ALL);

            mCurrentTouchLocked = lockTouch;
            updateStylusFingerOnlyModeFromSettings();
        }
    }

    public void updateStylusFingerOnlyModeFromSettings() {
        boolean fingerOnly;
        int setting = getStylusFingerOnlyModeSetting(0);

        if (mCurrentTouchLocked || (setting != 0)) {
            fingerOnly = false;
        }
        else {
            fingerOnly = mAppStylusFingerOnlyMode;
        }

        setStylusFingerOnlyMode(fingerOnly);
    }

    public void updateStylusLeftHandedModeFromSettings() {
        int setting = getStylusLeftHandedModeSetting(0);
        nativeSetStylusLeftHandedMode(mPtr, setting != 0);
    }
    ...
    private void registerShowStylusSettingObserver() {
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(Settings.System.SHOW_STYLUS), true,
                new ContentObserver(mHandler) {
                    @Override
                    public void onChange(boolean selfChange) {
                        updateShowStylusFromSettings();
    ...
    private void registerShowEraserSettingObserver() {
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(Settings.System.SHOW_ERASER), true,
                new ContentObserver(mHandler) {
                    @Override
                    public void onChange(boolean selfChange) {
                        updateShowEraserFromSettings();
    ...
    private void registerPenToTouchDelayMsSettingObserver() {
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(Settings.System.PEN_TO_TOUCH_DELAY_MS), true,
                new ContentObserver(mHandler) {
                    @Override
                    public void onChange(boolean selfChange) {
                        updatePenToTouchDelayMsFromSettings();
    ...
    private void registerStylusFingerOnlyModeSettingObserver() {
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(Settings.System.ENABLE_STYLUS), true,
                new ContentObserver(mHandler) {
                    @Override
                    public void onChange(boolean selfChange) {
                        updateStylusFingerOnlyModeFromSettings();
    ...
    private void registerStylusLeftHandedModeSettingObserver() {
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(Settings.System.STYLUS_LEFT_HANDED), true,
                new ContentObserver(mHandler) {
                    @Override
                    public void onChange(boolean selfChange) {
                        updateStylusLeftHandedModeFromSettings();
    ...
    private int getShowStylusSetting(int defaultValue) {
        int result = defaultValue;
        try {
            result = Settings.System.getIntForUser(mContext.getContentResolver(),
                    Settings.System.SHOW_STYLUS, UserHandle.USER_CURRENT);
        } catch (SettingNotFoundException snfe) {
        }
        return result;
    }

    private int getShowEraserSetting(int defaultValue) {
        int result = defaultValue;
        try {
            result = Settings.System.getIntForUser(mContext.getContentResolver(),
                    Settings.System.SHOW_ERASER, UserHandle.USER_CURRENT);
        } catch (SettingNotFoundException snfe) {
        }
        return result;
    }

    private int getPenToTouchDelayMsSetting(int defaultValue) {
        int result = defaultValue;
        try {
            result = Settings.System.getIntForUser(mContext.getContentResolver(),
                    Settings.System.PEN_TO_TOUCH_DELAY_MS, UserHandle.USER_CURRENT);
        } catch (SettingNotFoundException snfe) {
        }
        return result;
    }

    private int getStylusFingerOnlyModeSetting(int defaultValue) {
        int result = defaultValue;
        try {
            result = Settings.System.getIntForUser(mContext.getContentResolver(),
                    Settings.System.ENABLE_STYLUS, UserHandle.USER_CURRENT);
        } catch (SettingNotFoundException snfe) {
        }
        return result;
    }

    private int getStylusLeftHandedModeSetting(int defaultValue) {
        int result = defaultValue;
        try {
            result = Settings.System.getIntForUser(mContext.getContentResolver(),
                    Settings.System.STYLUS_LEFT_HANDED, UserHandle.USER_CURRENT);
        } catch (SettingNotFoundException snfe) {
        }
        return result;
    }

    private BroadcastReceiver mTouchLockStateReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if(WindowManagerPolicy.ACTION_LOCK_TOUCH.equals(intent.getAction())) {
                boolean lockTouch = intent.getBooleanExtra(
                    WindowManagerPolicy.EXTRA_TOUCH_LOCK, false);
                updateTouchLockState(lockTouch);
            }
        }
    };

    private BroadcastReceiver mEnableStylusStateReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if(WindowManagerPolicy.ACTION_ENABLE_STYLUS.equals(intent.getAction())) {
                String stylusEnablePackage = intent.getStringExtra(
                    WindowManagerPolicy.EXTRA_ENABLE_STYLUS_PACKAGE);

                if (DEBUG) Slog.d(TAG, "mEnableStylusStateReceiver: package - "
                                + stylusEnablePackage);

                if (stylusEnablePackage != null && stylusEnablePackage.isEmpty()) {
                    stylusEnablePackage = null;
                }

                if (mStylusEnablePackage != null && stylusEnablePackage == null) {
                    mStylusEnablePackage = null;
                    setAppStylusFingerOnlyMode(true);
                }
                else if (stylusEnablePackage != null &&
                        !stylusEnablePackage.equals(mStylusEnablePackage)) {
                    mStylusEnablePackage = stylusEnablePackage;
                    setAppStylusFingerOnlyMode(false);
                }
            }
        }
    };
    ...
    // ensure stylus insertion/removal event(SW_TABLET_MODE) is disabled
    // until the current user makes it through the setup wizard
    private void updateUserSetupFromSettings() {
        mUserSetup = 0 != Settings.Secure.getIntForUser(
    ...
    private void registerUserSetupObserver() {
        mContext.getContentResolver().registerContentObserver(
    ...
    private void notifySwitch(long whenNanos, int switchValues, int switchMask) {
        ...
        if (mUserSetup && ((switchMask & SW_TABLET_MODE_BIT) != 0)) {
            final boolean penRemoved = ((switchValues & SW_TABLET_MODE_BIT) != 0);
            mWindowManagerCallbacks.notifyStylusSwitchChanged(penRemoved);
        }
    ...
    public interface WindowManagerCallbacks {
        ...
        public void notifyStylusSwitchChanged(boolean penRemoved);
