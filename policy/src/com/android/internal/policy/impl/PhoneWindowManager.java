...
import com.android.internal.widget.JoystickLocationView;
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private final class JoystickLocationJoystickEventListener implements JoystickEventListener {
        @Override
        public void onJoystickEvent(MotionEvent motionEvent) {
            if (mJoystickLocationView != null) {
                mJoystickLocationView.addJoystickEvent(motionEvent);
            }
        }

        @Override
        public void onGamepadEvent(KeyEvent keyevent) {
            if (mJoystickLocationView != null) {
                mJoystickLocationView.addKeyEvent(keyevent);
            }
        }
    }
    ...
    // Joystick location view state
    JoystickLocationJoystickEventListener mJoystickEventListener;
    JoystickLocationView mJoystickLocationView;
    ...
    private static final int MSG_ENABLE_JS_LOCATION = 5;
    private static final int MSG_DISABLE_JS_LOCATION = 6;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_DISPATCH_MEDIA_KEY_REPEAT_WITH_WAKE_LOCK:
                    ...
                    break;
                case MSG_ENABLE_JS_LOCATION:
                    enableJoystickLocation();
                    break;
                case MSG_DISABLE_JS_LOCATION:
                    disableJoystickLocation();
                    break;
            }
        }
    }
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.System.getUriFor(
                    ...
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.System.getUriFor(
                    Settings.System.JOYSTICK_LOCATION), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.Secure.getUriFor(
    ...
    private void interceptPowerKeyDown(boolean handled) {
        ...
        if (!handled) {
            ...
            mHandler.postDelayed(mPowerExtraLongPress, SystemProperties.getInt("persist.nvidia.power_off_delay", 5000));
        }
    }

    private boolean interceptPowerKeyUp(boolean canceled) {
        mHandler.removeCallbacks(mPowerExtraLongPress);
        if (!mPowerKeyHandled) {
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        mFullscreenObserver = new FullscreenObserver(mHandler);
        mFullscreenObserver.observe();
    ...
    public void setInitialDisplaySize(Display display, int width, int height, int density) {
        ...
        int overrideId = res.getIdentifier("android:config_navBarCanMove", "bool", null);
        if (overrideId > 0) {
            mNavigationBarCanMove = res.getBoolean(overrideId);
        }
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            if (mSystemReady) {
                int joystickLocation = Settings.System.getIntForUser(resolver,
                        Settings.System.JOYSTICK_LOCATION, 0, UserHandle.USER_CURRENT);
                if (mJoystickLocationMode != joystickLocation) {
                    mJoystickLocationMode = joystickLocation;
                    mHandler.sendEmptyMessage(joystickLocation != 0 ?
                            MSG_ENABLE_JS_LOCATION : MSG_DISABLE_JS_LOCATION);
                }
            }
    ...
    private void enableJoystickLocation() {
        if (mJoystickLocationView == null) {
                mJoystickLocationView = new JoystickLocationView(mContext);
                ...
                lp.setTitle("JoystickLocation");
                WindowManager wm = (WindowManager)
                    mContext.getSystemService(Context.WINDOW_SERVICE);
                lp.inputFeatures |= WindowManager.LayoutParams.INPUT_FEATURE_NO_INPUT_CHANNEL;
                wm.addView(mJoystickLocationView, lp);

                mJoystickEventListener = new JoystickLocationJoystickEventListener();
                mWindowManagerFuncs.registerJoystickEventListener(mJoystickEventListener);
        }
    }

    private void disableJoystickLocation() {
        if (mJoystickEventListener != null) {
            mWindowManagerFuncs.unregisterJoystickEventListener(
                    mJoystickEventListener);
        ...
        if (mJoystickLocationView != null) {
            WindowManager wm = (WindowManager)
                    mContext.getSystemService(Context.WINDOW_SERVICE);
            wm.removeView(mJoystickLocationView);
            mJoystickLocationView = null;
        }
    }
    ...
    public int getNonDecorDisplayWidth(int fullWidth, int fullHeight, int rotation) {
        if (mHasNavigationBar && mFakeImmersive == false) {
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            if (down) {
                if (mImmersiveModeConfirmation.confirmCurrentPrompt()) {
                    // We dismissed, so discard the event instead of passing it along
                    return -1;
                }
            }
        }
    ...
    public void getContentInsetHintLw(WindowManager.LayoutParams attrs, Rect contentInset) {
        final int fl = getFullscreenFlags(attrs.flags);
            ...
            if (mNavigationBar != null) {
                ...
                if (mNavigationBarOnBottom) {
                    ...
                    if (mFakeImmersive == false) {
                        ...
                        mStableBottom = mStableFullscreenBottom = displayWidth - overscanRight;
                    }
    ...
    public void layoutWindowLw(WindowState win, WindowManager.LayoutParams attrs,
        ...
        final int sysUiFl = getFullscreenUiVisibility(win.getSystemUiVisibility());
    ...
    public int finishPostLayoutPolicyLw() {
        ...
        if (mStatusBar != null) {
            ...
            if (mForceStatusBar || mForceStatusBarFromKeyguard) {
                ...
                topIsFullscreen = (getFullscreenFlags(lp.flags) & WindowManager.LayoutParams.FLAG_FULLSCREEN) != 0
    ...
    public void notifyStylusSwitchChanged(boolean penRemoved) {
        ...
        Intent intent = new Intent(ACTION_PEN_CHANGED);
        if (penRemoved) {
            intent.putExtra(EXTRA_PEN_STATE, PEN_STATE_REMOVED);
            if (DEBUG_INPUT) Slog.d(TAG, "notifyStylusSwitchChanged: PEN_REMOVED");
        }
        else {
            intent.putExtra(EXTRA_PEN_STATE, PEN_STATE_INSERTED);
            if (DEBUG_INPUT) Slog.d(TAG, "notifyStylusSwitchChanged: PEN_INSERTED");
        }
            ...
            case KeyEvent.KEYCODE_POWER: {
                ...
                if (down) {
                    ...
                    if (interceptPowerKeyUp(canceled || mPendingPowerKeyUpCanceled)) {
                        // Since we are going to sleep by power button, unlock touch
                        Intent intent = new Intent(ACTION_LOCK_TOUCH);
                        intent.putExtra(EXTRA_TOUCH_LOCK, false);
                        mContext.sendBroadcastAsUser(intent, UserHandle.ALL);

                        if (keyCode == KeyEvent.KEYCODE_POWER) {
                            result = (result & ~ACTION_WAKE_UP) | ACTION_GO_TO_SLEEP;
                        } else {
                            result = handleStbPowerKey();
                        }
                    }
    ...
    private int updateSystemUiVisibilityLw() {
        ...
        int tmpVisibility = getFullscreenUiVisibility(win.getSystemUiVisibility())
    ...
    private int updateSystemBarsLw(WindowState win, int oldVis, int vis) {
        ...
        boolean hideStatusBarWM =
                mTopFullscreenOpaqueWindowState != null &&
                (getFullscreenFlags(mTopFullscreenOpaqueWindowState.getAttrs().flags)
        ...
        if (win != null && oldImmersiveMode != newImmersiveMode) {
            ...
            mImmersiveModeConfirmation.immersiveModeChanged(getFullscreenPackageName(pkg), newImmersiveMode);
        }
    ...
    private final Runnable mPowerExtraLongPress = new Runnable() {
        @Override
        public void run() {
            mPowerKeyHandled = true;
            performHapticFeedbackLw(null, HapticFeedbackConstants.LONG_PRESS, false);
    ...
    private int handleStbPowerKey() {
        if ((mGlobalActions != null) && mGlobalActions.isDialogShowing()) {
            mGlobalActions.dismissDialog();
        } else if (isDeviceProvisioned() && !isKeyguardLocked()) {
            // Skip this if the device is not provisioned yet, or if the
            // keyguard is blocking the screen
            sendCloseSystemWindows();
            Intent close = new Intent("com.android.systemui.recent.action.CLOSE");
            close.setPackage("com.android.systemui");
            try {
                mContext.sendBroadcastAsUser(close, UserHandle.CURRENT);
            } catch (ActivityNotFoundException ex) {
                Slog.w(TAG, "Failed RecentsActivity.CLOSE_RECENTS_INTENT");
            }

            Intent intent =  new Intent(Intent.ACTION_MAIN, null);
            intent.addCategory("android.intent.category.STB_POWER");
    ...
    private FullscreenObserver mFullscreenObserver;
    private boolean mFakeImmersive = false;
    private class FullscreenObserver extends ContentObserver {
        FullscreenObserver(Handler handler) {
            super(handler);
        }

        void observe() {
            ContentResolver resolver = mContext.getContentResolver();
            mFakeImmersive = Settings.System.getIntForUser(mContext.getContentResolver(), Settings.System.FULLSCREEN_MODE, 0, UserHandle.USER_CURRENT) != 0;
            if (mImmersiveModeConfirmation != null) {
                mImmersiveModeConfirmation.immersiveModeChanged("nv_fullscreen", mFakeImmersive);
            }
            resolver.registerContentObserver(Settings.System.getUriFor(Settings.System.FULLSCREEN_MODE), false, this, UserHandle.USER_ALL);
        }
    ...
    private int getFullscreenFlags(int flags) {
        if (mFakeImmersive == false || (flags & FLAG_FORCE_NOT_FULLSCREEN) != 0) {
            return flags;
        }
        return flags | FLAG_FULLSCREEN;
    }

    private int getFullscreenUiVisibility(int vis) {
        if (mFakeImmersive) {
            vis |= View.SYSTEM_UI_FLAG_LAYOUT_STABLE | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION | View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY | View.SYSTEM_UI_FLAG_FULLSCREEN;
        }
        return vis;
    }

    private String getFullscreenPackageName(String pkg) {
        if (mFakeImmersive) {
            return "nv_fullscreen";
        }
        return pkg;
    }
}
