...
public class WifiStateMachine extends StateMachine {
    ...
    private final AtomicBoolean mMiracastMode = new AtomicBoolean(false);
    private final AtomicBoolean bAmpduDisabled = new AtomicBoolean(false);
    ...
    */
     ...
     */
    private static final int SUPPLICANT_RESTART_INTERVAL_MSECS = 15000;
    ...
    /* Nv custom filters*/
    static final int CMD_NV_FLT_CONFIG                    = BASE + 89;
   ...
   /* Set the frequency band */
    ...
    public static final int CONNECT_WIFI_USER_REJECT      = BASE + 138;
    public static final int CONNECT_WIFI_USER_ACCEPT      = BASE + 139;
    ...
    public static final int P2P_GROUP_REMOVE_SUCCESS      = BASE + 143;
    public static final int P2P_GROUP_REMOVE_FAILURE      = BASE + 144;
    public static final int CMD_SET_STA_LOW_LATENCY       = BASE + 145;
    public static final int ENABLE_MIRACAST_MODE          = BASE + 146;
    ...
    public static final int CMD_SET_DISABLE_AMPDU         = BASE + 142;
    ...
    /* P2P and WiFi frequecy conflict state*/
    private State mFrequencyConflictState = new FrequencyConflictState();
        ...
        mContext.registerReceiver(
                new BroadcastReceiver() {
                    ...
                    public void onReceive(Context context, Intent intent) {
                        ...
                        /* Disabling the scan when there is an active P2P connection */
            			if(!mMiracastMode.get())
            ...
            addState(mSupplicantStartedState, mDefaultState);
                ...
                addState(mDriverStartedState, mSupplicantStartedState);
                    ...
                    addState(mConnectModeState, mDriverStartedState);
                        ...
                        addState(mFrequencyConflictState, mConnectModeState);
    ...
    /**
     * Configure NV custom filters
     */
    public boolean configNvFilters(AsyncChannel channel, int command, String value) {
        if (!mIsRunning) {
            logd("configNvFilters(): not configurable due to Wifi not running");
            return false;
        }
        Message resultMsg = channel.sendMessageSynchronously(obtainMessage(CMD_NV_FLT_CONFIG,
                command, 0, value));
        boolean result = (resultMsg.arg1 != FAILURE);
        resultMsg.recycle();
        return result;
    ...
    private void disableWifiBasedLocationServices(boolean disable) {
        boolean bEnabled = Settings.Secure.isLocationProviderEnabled(mContext.getContentResolver(),
                                   LocationManager.NETWORK_PROVIDER);
        if (bEnabled) {
            Settings.Secure.setLocationProviderEnabled(mContext.getContentResolver(),
                    LocationManager.NETWORK_PROVIDER, disable);
        }
    }

    public void setStaLowLatency(boolean enable) {
        /* only disable location service else pop-up will be displayed */
        if (enable) {
            disableWifiBasedLocationServices(!enable);
        }
        sendMessage(obtainMessage(CMD_SET_STA_LOW_LATENCY, enable ? 1 : 0, 0));
    }

    public void setDisableAmpdu(boolean enable) {
        if (enable) {
            disableWifiBasedLocationServices(!enable);
        }
        sendMessage(obtainMessage(CMD_SET_DISABLE_AMPDU, enable ? 1 : 0, 0));
    }
    ...
    public int getMaxLinkSpeed() {
        String maxSpeedStr = mWifiConfigStore.getMaxLinkSpeed();
        log("Max Speed String:" + maxSpeedStr);
        if (maxSpeedStr != null) {
            String[] prop = maxSpeedStr.split(" ");
            if (prop.length < 2) {
                return -1;
            } else if (prop[0].equals("MaxLinkSpeed")) {
                int maxSpeed = Integer.parseInt(prop[1]);
                if (DBG) log("Max Speed: " + maxSpeed);
                return maxSpeed;
            } else {
                return -1;
            }
        ...
        public boolean processMessage(Message message) {
            switch (message.what) {
                ...
                case CMD_IP_ADDRESS_REMOVED:
                    ...
                    break;
                case CMD_SET_STA_LOW_LATENCY:
                    if (message.arg1 == 1) {
                        mWifiNative.startMiracastMode(2);
                    } else {
                        mWifiNative.startMiracastMode(0);
                    }
                    break;
                case ENABLE_MIRACAST_MODE:
                    int mode = message.arg1;
                    mMiracastMode.set(mode == 1);
                    break;
                case CMD_NV_FLT_CONFIG:
                    // This message comes here ONLY if DriverStartedState has no chance to handle it
                    loge("CMD_NV_FLT_CONFIG not handled by driver");
                    replyToMessage(message, CMD_NV_FLT_CONFIG, FAILURE);
                    break;
                default:
    ...
    class DriverStartedState extends State {
        ...
        public boolean processMessage(Message message) {
            switch(message.what) {
                ...
                case CMD_NV_FLT_CONFIG:
                    // configure NV custom filters
                    log("DRIVER NV-FLT-CONFIG " + message.arg1 + " " + (String)message.obj);
                    boolean ok = mWifiNative.configNvFilters(message.arg1, (String)message.obj);
                    replyToMessage(message, CMD_NV_FLT_CONFIG, ok ? SUCCESS : FAILURE);
                    break;
                default:
                    return NOT_HANDLED;
            }
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case WifiManager.CONNECT_NETWORK:
                    ...
                    /* arg2 is always greater than zero if its a request throught WifiManager
                     * When there is a request through manager set priority to p2p.
                     * in any other case set priority to sta.
                     * this will ensure FREQ_CONFLICT management in framework.
                     */
                    mCurrentNetworkId = netId;
                    int priority = 0;
                    if (mCurrentNetworkId == -1) {
                        for (WifiConfiguration temp : mWifiConfigStore.getConfiguredNetworks()) {
                            for (ScanResult result: mScanResults) {
                                String compareSSID = "\"" + result.SSID + "\"";
                                if (DBG) loge("result ssid = " + compareSSID);
                                if (temp.SSID.equals(compareSSID)) {
                                    if (DBG) loge("remembered network in scan results..trying to connect to " + temp.networkId);
                                    if (temp.priority >= priority) {
                                        mCurrentNetworkId = netId =  temp.networkId;
                                        priority = temp.priority;
                                        if (DBG) loge("priority is set to " + priority);
                                    }
                                } else
                                    if (DBG) loge("results donot contain " + temp.SSID);
                            }
                        }
                    }
                    ...
                    if (mWifiConfigStore.selectNetwork(netId) &&
                        ...
                        loge("Failed to connect config: " + config + " netId: " + mCurrentNetworkId);
                        replyToMessage(message, WifiManager.CONNECT_NETWORK_FAILED,
                                WifiManager.ERROR);
                        break;
                    }
                    break;
                case WifiMonitor.FREQUENCY_CONFLICT_EVENT:
                     if (DBG) log("Frequency conflict event received in " + getName());
    ...
    class FrequencyConflictState extends State {
        ...
        public void enter() {
            if (DBG) log(getName() + "\n");
            if (mWifiP2pManager.isAutonomousGroupOwner()) {
                log(getName() + "Stopping Autonomous GO because of frequency conflict\n");
                Intent intent = new Intent("com.nvwfd.action.FREQ_CONFLICT_DISCONNECT");
                mContext.sendBroadcast(intent);
                sendMessage(CONNECT_WIFI_USER_ACCEPT);
                return ;
            }
        ...
        private void notifyFrequencyConflict() {
            if (DBG) log("Notify Wifi-P2P frequency conflict to user");
            Resources r = Resources.getSystem();

            AlertDialog dialog = new AlertDialog.Builder(mContext)
                .setMessage(r.getString(R.string.p2p_wifi_frequency_conflict_message))
                .setPositiveButton(r.getString(R.string.dlg_ok), new OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            //To Notify NvwfdService that disconnection is due to frequency conflict
                            Intent intent = new Intent("com.nvwfd.action.FREQ_CONFLICT_DISCONNECT");
                            mContext.sendBroadcast(intent);
                            sendMessage(CONNECT_WIFI_USER_ACCEPT);
                        }
                ...
                .setNegativeButton(r.getString(R.string.decline), new OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            sendMessage(CONNECT_WIFI_USER_REJECT);
                        }
                    })
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                case CONNECT_WIFI_USER_ACCEPT:
                    mWifiP2pChannel.sendMessage(obtainMessage(WifiP2pManager.REMOVE_GROUP, 1, 0));
                    break;
                case CONNECT_WIFI_USER_REJECT:
                    transitionTo(mConnectModeState);
                    sendMessage(CMD_DISCONNECT);
                    break;
                case P2P_GROUP_REMOVE_SUCCESS:
                    transitionTo(mConnectModeState);
                    sendMessage(obtainMessage(WifiManager.CONNECT_NETWORK, mCurrentNetworkId, 0));
                    break;
                case P2P_GROUP_REMOVE_FAILURE:
                case WifiP2pManager.REMOVE_GROUP_FAILED:
                    loge(getName() + "p2p group remove failed!");
                    break;
                default:
                    return NOT_HANDLED;
            }
        ...
        public boolean processMessage(Message message) {
            switch (message.what) {
               case WifiWatchdogStateMachine.POOR_LINK_DETECTED:
                    ...
                    transitionTo(mVerifyingLinkState);
                    break;
               case CMD_SET_DISABLE_AMPDU:
                    if (message.arg1 == 1) {
                        mWifiNative.startMiracastMode(3);
                        mWifiNative.reassoccurrent();
                    } else {
                        mWifiNative.startMiracastMode(4);
                        mWifiNative.reassoccurrent();
                    }
                    break;
                default:
