...
public class WifiP2pService extends IWifiP2pManager.Stub {
    ...
    private static final int GROUP_IDLE_TIME_S = 30;
    ...
    private boolean mTemporaryP2pDisconnect = false;
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_GROUP_FORMATION_FAILURE_EVENT:
                    break;
                case WifiP2pManager.CMD_SET_MIRACAST_MODE:
                    int mode = message.arg1;
                    if (mWifiNative.startMiracastMode(mode) != -1) {
                        mWifiChannel.sendMessage(obtainMessage(WifiStateMachine.ENABLE_MIRACAST_MODE, mode, 0));
                        replyToMessage(message, WifiP2pManager.MIRACAST_MODE_SUCCESS);
                    }
                    else
                        replyToMessage(message, WifiP2pManager.MIRACAST_MODE_FAILED);
                    break;
                default:
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_GROUP_STARTED_EVENT:
                    ...
                    if (mGroup.getNetworkId() == WifiP2pGroup.PERSISTENT_NET_ID) {
                        ...
                        loge("Unexpected group creation, remove " + mGroup);
                        loge("Skip group removal on network ID -1");
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.FREQUENCY_CONFLICT_EVENT:
                    loge(getName() + "group negotiation failed due to frequency conflict!");
                    transitionTo(mFrequencyConflictState);
                    break;
                default:
                    return NOT_HANDLED;
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.AP_STA_DISCONNECTED_EVENT:
                    ...
                    sendP2pStaDisconnectBroadcast(device);
                    if (deviceAddress != null) {
                ...
                case DhcpStateMachine.CMD_POST_DHCP_ACTION:
                    ...
                    break;
                case WifiP2pManager.REMOVE_GROUP:
                    mTemporaryP2pDisconnect = (message.arg1 == 1);
                    if (DBG) logd(getName() + " remove group(message from wifistatemachine?" + mTemporaryP2pDisconnect + ")");
                    if (mWifiNative.p2pGroupRemove(mGroup.getInterface())) {
                        ...
                        handleGroupRemoved();
                        mTemporaryP2pDisconnect = false;
                        transitionTo(mInactiveState);
                        replyToMessage(message, WifiP2pManager.REMOVE_GROUP_FAILED,
                                WifiP2pManager.ERROR);
                    }
                    break;
                 /* For autonomous group owner cases disconnect and cancel connect cases should
                  * be handled
                  */
                 case WifiP2pManager.DISCONNECT:
                    WifiP2pConfig mConfig = (WifiP2pConfig) message.obj;
                    logd(getName() + "WifiP2pManager.DISCONNECT" + mConfig.deviceAddress);
                    mWifiNative.p2pDisconnect(mConfig);
                    break;
                 case WifiP2pManager.CANCEL_CONNECT:
                    logd(getName() + "Cancelling on going connection");
                    mWifiNative.p2pCancelConnect();
                    replyToMessage(message, WifiP2pManager.CANCEL_CONNECT_SUCCEEDED);
                    break;
                ...
                case WifiMonitor.P2P_GROUP_REMOVED_EVENT:
                    ...
                    if (mTemporaryP2pDisconnect) {
                        mWifiChannel.sendMessage(WifiStateMachine.P2P_GROUP_REMOVE_SUCCESS);
                        mTemporaryP2pDisconnect = false;
                    }
                    handleGroupRemoved();
                    transitionTo(mInactiveState);
   ...
   private void sendP2pStaDisconnectBroadcast(WifiP2pDevice device) {
       if (DBG) logd("sending p2p sta disconnect broadcast");
       Intent intent = new Intent(WifiP2pManager.WIFI_P2P_STA_DISCONNECT);
    ...
    private void notifyInvitationReceived() {
        String dialogTitle;
        String dialogPositiveButtonLabel;
        String dialogNegativeButtonLabel;
        Resources r = Resources.getSystem();
        ...
        if(isInvitationFromMiracastDevice(dev)) {
            dialogTitle = r.getString(R.string.miracast_pairing_request);
            dialogPositiveButtonLabel = r.getString(R.string.miracast_label_pair);
            dialogNegativeButtonLabel = r.getString(R.string.miracast_label_cancel);
            addRowToDialogForMiracastDevice(group, getDeviceName(
                mSavedPeerConfig.deviceAddress));
        } else {
            dialogTitle = r.getString(R.string.wifi_p2p_invitation_to_connect_title);
            dialogPositiveButtonLabel = r.getString(R.string.accept);
            dialogNegativeButtonLabel = r.getString(R.string.decline);
        ...
        mPinEntryDialog = new AlertDialog.Builder(mContext)
            ...
            .setView(textEntryView)
            .setPositiveButton(dialogPositiveButtonLabel, new OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
            ...
            .setNegativeButton(dialogNegativeButtonLabel, new OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
    ...
    private void initializeP2pSettings() {
        ...
        mWifiNative.setConcurrencyPriority("p2p");
    ...
    private boolean isInvitationFromMiracastDevice(WifiP2pDevice device) {
        if(device != null && device.wfdInfo != null &&
           (device.wfdInfo.getDeviceType() == WifiP2pWfdInfo.WFD_SOURCE ||
            device.wfdInfo.getDeviceType() == WifiP2pWfdInfo.SOURCE_OR_PRIMARY_SINK)){
            return true;
        }
        return false;
    }

    private void addRowToDialogForMiracastDevice(ViewGroup group, String value) {
        Resources r = Resources.getSystem();
        String text = String.format(r.getString(
                            R.string.miracast_source_is_requesting_to_pair), value);
        View row = LayoutInflater.from(mContext).inflate(R.layout.wifi_p2p_dialog_row_label,
                group, false);
        ((TextView) row.findViewById(R.id.name)).setVisibility(View.GONE);
        ((TextView) row.findViewById(R.id.value)).setText(text);
        group.addView(row);
    }
}
