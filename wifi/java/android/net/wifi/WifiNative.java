...
public class WifiNative {
    ...
    public boolean enableNetworkNoConnect(int netId) {
    ...
    public boolean reassoccurrent() {
        return doBooleanCommand("REASSOCCURRENT");
    }
    ...
    /**
     * Add custom command by Nvidia.
     * @return {@code true} if the operation succeeded, {@code false} otherwise
     */
    public boolean configNvFilters(int command, String value) {
        return doBooleanCommand("DRIVER NV-FLT-CONFIG " + command + " " + value);
    }

    /**
     * Start/Stop WiFi driver low latency mode
     */
    public int startMiracastMode(int mode) {
        return doIntCommand("DRIVER SETMIRACAST " + mode);
    }

    public String getMaxLinkSpeed() {
        return doStringCommand("DRIVER MAXLINKSPEED");
    }
   ...
   /**
    ...
    public boolean p2pDisconnect(WifiP2pConfig config) {
        return doBooleanCommand("DEAUHENTICATE " + config.deviceAddress);
    }
